...
class RouterManager:
    ...
    def get_ospf_route(self, tn: telnetlib.Telnet) -> Optional[str]:
        ...
        try:
            ...
            ospf_ip = self.parse_routing_table(routing_output, tn.host, tn.port)
            return ospf_ip
        except Exception as e:
            logging.error(f"[{tn.host}:{tn.port}] 获取OSPF路由表时出错: {e}")
            return None

    def parse_routing_table(self, routing_table: str, host: str, port: int) -> Optional[str]:
        """
            ...
            for route_type in self.route_types:
                if route_type in line:
                    ...
                    if parts:
                        ...
                        logging.debug(f"[{host}:{port}] Found {route_type} route: {dest_ip}")
                        return dest_ip
        logging.warning(f"[{host}:{port}] 未找到 IS_L1、O_INTRA 或 BGP 路由")
        return None
    ...
    def perform_nqa_test(self, tn: telnetlib.Telnet, dest_ip: str, max_attempts: int = 5) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            while attempt_count < max_attempts:
                ...
                if partial_output.strip():  # 如果有任何输出
                    result += partial_output
                    logging.info(f"[{host}:{port}] 获取到 NQA 测试结果（第 {attempt_count + 1} 次尝试）")
                    break
                else:
                    attempt_count += 1
                    logging.debug(f"[{host}:{port}] 第 {attempt_count} 次尝试获取 NQA 结果...")

                    if attempt_count >= max_attempts:
                        logging.warning(f"[{host}:{port}] 达到最大尝试次数 ({max_attempts})，NQA测试结果可能不完整。")
                        break

            # 记录并返回结果
            logging.debug(f"[{host}:{port}] NQA 测试结果:\n{result}")
            ...
            # 解析并返回性能评估输入
            metrics = self.parse_nqa_result(result, host, port)
            if metrics is None:
                logging.warning(f"[{host}:{port}] 解析NQA结果失败。")
            return metrics

        except Exception as e:
            logging.error(f"[{host}:{port}] 执行NQA测试时出错: {e}")
            return None


    def parse_nqa_result(self, nqa_result: str, host: str, port: int) -> Optional[Dict[str, Any]]:
        """解析 NQA 结果，提取性能指标。"""
        metrics = {
            "延迟": None,
            "抖动": None,
        ...
        for line in nqa_result.splitlines():
            ...
            if rtt_match:
                try:
                    ...
                    logging.debug(f"[{host}:{port}] 解析延迟 (平均 RTT): {latency_avg} ms")
                except ValueError as e:
                    logging.error(f"[{host}:{port}] 无法解析延迟，行: '{line}'。错误: {e}")
                    metrics["延迟"] = None
            ...
            if packet_loss_match:
                try:
                    ...
                    logging.debug(f"[{host}:{port}] 解析丢包率: {packet_loss} %")
                except ValueError as e:
                    logging.error(f"[{host}:{port}] 无法解析丢包率，行: '{line}'。错误: {e}")
                    metrics["丢包率"] = None
            ...
            if jitter_avg_match:
                try:
                    ...
                    logging.debug(f"[{host}:{port}] 解析抖动 (平均抖动): {jitter_value} ms")
                except ValueError as e:
                    logging.error(f"[{host}:{port}] 无法解析抖动，行: '{line}'。错误: {e}")
                    metrics["抖动"] = None

            # 解析单向延迟的最大和最小延迟
            one_way_max_sd_match = one_way_max_sd_pattern.search(line)
            if one_way_max_sd_match:
                logging.debug(f"[{host}:{port}] 解析最大单向延迟: {one_way_max_sd_match.group(1)} ms")
            
            one_way_min_sd_match = one_way_min_sd_pattern.search(line)
            if one_way_min_sd_match:
                logging.debug(f"[{host}:{port}] 解析最小单向延迟: {one_way_min_sd_match.group(1)} ms")

        # 如果所有指标都无法解析，记录原始 NQA 结果以便调试
        if all(value is None for value in metrics.values()):
            logging.warning(f"[{host}:{port}] 所有性能指标均为 None。原始 NQA 结果:\n{nqa_result}")
