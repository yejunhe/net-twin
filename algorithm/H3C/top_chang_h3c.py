    ...
    def parse_display_ip_interface_brief(self, output: str) -> List[Dict[str, str]]:
        ...
        for line in lines:
            ...
            if not header_found:
                ...
                if match:
                    ip_address = match.group('ip_address').replace('unassigned', '--')  # 替换 'unassigned' 为 '--'
                    vpn = match.group('vpn').replace('unassigned', '--')  # 替换 'unassigned' 为 '--'
                    description = match.group('description').strip().replace('unassigned', '--')  # 替换 'unassigned' 为 '--'

                    # 仅添加 IP Address/Mask 不为 '--' 的接口
                    if ip_address != '--':
                        interface_info = {
                        ...
                        logging.debug(f"接口 {match.group('interface')} 的 IP Address/Mask 为 '--'，跳过。")
                else:
                    logging.debug(f"在 'display ip interface brief' 中未匹配的行: {line}")
                    continue

        logging.debug(f"解析后的 Telnet 接口列表（仅含有 IP 配置的接口）: {interfaces}")
        return interfaces
