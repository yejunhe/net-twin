...
class NodeReader:
    def __init__(self, input_file_path, required_version):
        ...
        self.output_data = {
            ...
            "links": [],
            "containers": [],
            "performance_evaluation": ""  # 添加性能评估字段
        }
    ...
    def evaluate_performance(self):
        ...
        if average_throughput < 300 or cpu_overload or memory_overload:
            self.output_data["performance_evaluation"] = "Poor"
        elif 300 <= average_throughput < 700 and not cpu_overload and not memory_overload:
            self.output_data["performance_evaluation"] = "Moderate"
        else:
            self.output_data["performance_evaluation"] = "Good"
    ...
    def write_output(self, output_file_path):
        ...
        with open(output_file_path, 'w') as file:
            file.write(self.format_output_data())
    ...
    def format_output_data(self):
        ...
        formatted_data.append("Nodes Information:")
        for node in self.output_data["nodes"]:
            formatted_data.append(f"Node ID: {node['node_id']}, Name: {node['node_name']}, Throughput: {node['throughput_mbps']} Mbps")

        formatted_data.append("\nLinks Information:")
        for link in self.output_data["links"]:
            formatted_data.append(f"Source: {link['source']['node_name']} ({link['source']['interface_name']}), Target: {link['target']['node_name']} ({link['target']['interface_name']})")

        formatted_data.append("\nContainers Information:")
        for container in self.output_data["containers"]:
            formatted_data.append(f"Container ID: {container['container_id']}, CPU Usage: {container['cpu_percentage']}, Memory Usage: {container['memory_usage']}")
            formatted_data.append(f"Router Name: {container['router_name']}, FRR Config Preview: {container['frr_conf_preview']}")

        formatted_data.append("\nPerformance Evaluation:")
        formatted_data.append(self.output_data["performance_evaluation"])
...
def main():
    ...
    output_file_name = 'node_link.txt'
    required_version = '2'
