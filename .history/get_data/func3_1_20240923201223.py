...
class RouterTelnetConnection:
    ...
    def send_command(self, command, wait_time=1):
        ...
        try:
            ...
            # 移除提示符
            prompt_str = self.prompt.decode('ascii').strip()
            if output.endswith(prompt_str):
                output = output[:-len(prompt_str)].strip()
        ...
        if protocol == 'isis':
            ...
            for line in output.splitlines():
                ...
                if in_peer_section:
                    ...
                    if len(columns) >= 1:
                        sys_id = columns[0]
                        if sys_id not in neighbors:
                            neighbors.append(sys_id)  # 提取唯一的System Id
        elif protocol == 'bgp':
            ...
            for line in output.splitlines():
                ...
                if line.startswith("Peer") and "AS" in line and "State" in line:
                    ...
                    continue
                if in_bgp_section:
                    if not line or line.startswith("BGP local"):
                        continue
                    peer_info = line.split()
                    if len(peer_info) >= 1:
                        neighbors.append(peer_info[0])  # 例如：邻居IP
        elif protocol == 'mpls_ldp':
            # 使用正则表达式匹配PeerID
            pattern = re.compile(r'^(\d{1,3}(?:\.\d{1,3}){3})\:\d+\s+')
            in_ldp_section = False
            for line in output.splitlines():
                line = line.strip()
                if line.startswith("PeerID") and "TransportAddress" in line:
                    in_ldp_section = True
                    continue
                if in_ldp_section:
                    if not line or "TOTAL" in line or line.startswith("---"):
                        break
                    match = pattern.match(line)
                    if match:
                        ldp_ip = match.group(1)
                        if ldp_ip not in neighbors:
                            neighbors.append(ldp_ip)  # 提取唯一的LDP Peer IP
        return neighbors
