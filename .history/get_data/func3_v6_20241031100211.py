...
class RouterTelnetManager:
    ...
    def get_sysname(self, connection):
        """
        Retrieve sysname from the router by checking the prompt.

        This method checks if the current prompt is in the format <sysname>. If not, it sends 'q' commands
        to exit undesired prompts until the correct format is detected or the maximum number of attempts is reached.

        :param connection: RouterTelnetConnection instance.
        :return: sysname string if found, else None.
        """
        try:
            attempts = 0
            max_attempts = 5
            while attempts < max_attempts:
                prompt = connection.prompt.decode('ascii').strip()
                if prompt.startswith('<') and prompt.endswith('>'):
                    sysname = prompt[1:-1]
                    return sysname
                elif prompt.startswith('[') and prompt.endswith(']'):
                    print(f"[{connection.host}:{connection.port}] Detected '[' ']' prompt. Sending 'q' command to exit.")
                    connection.send_command('q', wait_time=1)
                    attempts += 1
                else:
                    print(f"[{connection.host}:{connection.port}] Unrecognized prompt format: '{prompt}'.")
                    break
            print(f"[{connection.host}:{connection.port}] Failed to retrieve sysname after {max_attempts} attempts.")
            return None
        except Exception as e:
