...
class RouterTelnetConnection:
    def __init__(self, host, port, timeout=10):
        ...
        self.tn = None
        self.prompt = b'>'
        self.sysname = None  # Initialize sysname
    ...
    def connect(self):
        ...
        try:
            ...
            while True:
                ...
                if '<' in initial_output and '>' in initial_output:
                    ...
                    if start < end:
                        ...
                        self.sysname = prompt_str  # Set sysname
                    ...
                    # Send 'q' command and retry
                    print(f"Detected undesired prompt '[' ']' on {self.host}:{self.port}. Sending 'q' command.")
            ...
            if self.tn and self.sysname:
                self.tn.write(b'\n')  # Ensure at prompt
                self._read_until_prompt()
                print(f"Connected to {self.host}:{self.port} with sysname '{self.sysname}'")
            else:
                print(f"Failed to retrieve sysname for {self.host}:{self.port}")
                self.close()
...
class RouterTelnetManager:
    ...
    def connect_and_collect(self, node, protocols=('isis', 'bgp', 'mpls_ldp')):
        ...
        connection.connect()
        if not connection.tn or not connection.sysname:
            print(f"Skipping {host}:{port} due to connection issues or missing sysname.")
            return

        try:
            # Store sysname
            self.sysnames[f"{host}:{port}"] = connection.sysname
            print(f"[{host}:{port}] Retrieved sysname: {connection.sysname}")
    ...
    def parse_bgp_summary(self, output):
        ...
        for line in lines:
            ...
            if line.startswith("Total number of peers"):
                ...
                try:
                    # Extract total_peers
                    total_peers_index = parts.index("peers") + 2  # 'peers : <number>'
                ...
                try:
                    ...
                    established_index = parts.index("established") + 4  # 'established state : <number>'
