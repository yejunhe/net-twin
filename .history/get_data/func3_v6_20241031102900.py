...
class RouterTelnetManager:
    ...
    def connect_and_collect(self, node, protocols=('isis', 'bgp', 'mpls_ldp')):
        ...
        try:
            ...
            if sysname:
                ...
                sysname = "unknown"  # 如果未能获取sysname，使用默认值
            ...
            # 将sysname添加到neighbor_data字典中
            self.neighbor_data[f"{host}:{port} ({sysname})"] = node_neighbors
            ...
            if routing_output:
                ...
                # 将sysname添加到routing_tables字典中
                self.routing_tables[f"{host}:{port} ({sysname})"] = routing_entries
            ...
            if bgp_summary_output:
                ...
                # 将sysname添加到bgp_summaries字典中
                self.bgp_summaries[f"{host}:{port} ({sysname})"] = bgp_summary
    ...
    def send_scr_command(self, connection):
        """Send 'scr 0 t' command to the router only after detecting the prompt as '<>'."""
        try:
            while True:
                output = connection.send_command('', wait_time=1)  # 发送空命令以读取提示符
                lines = output.splitlines()
                if not lines:
                    print(f"[{connection.host}:{connection.port}] No output received while checking prompt.")
                    break
                ...
                if prompt.startswith('<') and prompt.endswith('>'):
                    ...
                    print(f"[{connection.host}:{connection.port}] Detected undesired prompt '{prompt}'. Sending 'q' command.")
                    connection.send_command('q', wait_time=1)
                else:
                    print(f"[{connection.host}:{connection.port}] Unrecognized prompt format: '{prompt}'. Retrying...")
                    connection.send_command('q', wait_time=1)  # Continue sending 'q' until prompt is correct
        except Exception as e:
