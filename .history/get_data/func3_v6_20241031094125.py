        ...
        try:
            ...
            while True:
                ...
                if '<' in initial_output and '>' in initial_output:
                    # Set prompt to include angle brackets
                    self.prompt = initial_output.split('<')[-1].split('>')[0].strip().encode('ascii') + b'> '
                    break
                elif '[' in initial_output and ']' in initial_output:
                    # Send 'quit' command and retry
                    print(f"Detected '[' ']' prompt on {self.host}:{self.port}. Sending 'quit' command.")
...
def main(input_path, output_path):
    ...
    # Initialize Telnet Manager and collect data
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Process network topology from param.json.")
