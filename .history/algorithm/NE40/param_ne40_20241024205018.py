...
class RouterManager:
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            ...
            # 提取 Telnet 获取的接口名称列表，并转换为统一的大小写
    ...
    def parse_display_ospf_interface(self, output: str) -> List[str]:
        ...
        # 正则表达式匹配接口行
        interface_regex = re.compile(r'^(?P<interface>\S+)\s+[\d\.]+')  # 简化的匹配接口名称
...
def write_interface_status(data_txt_path: str, mapping: Dict[str, Any]):
    ...
    节点: sysname1 (host:port)
        接口: Ethernet1/0/0接口配置状态: 已配置, OSPF已配置
        接口: Ethernet1/0/1接口配置状态: 已配置, OSPF未配置
    节点: sysname2 (host:port)
        接口: Ethernet1/0/2接口配置状态: 已配置, OSPF未配置
        接口: Loop0接口配置状态: 已配置, OSPF未配置
    """
    try:
