...
class NetworkManager:
    ...
    def collect_telnet_results(self) -> Dict[str, Any]:
        combined_checks = {}
    ...
    def get_nqa_metrics(self, host: str, port: int, dest_ip: str) -> Optional[Dict[str, Any]]:
        """
        通过 Telnet 执行 NQA 测试并获取性能指标。
        """
        try:
            ...
            logging.error(f"Error performing NQA test on {host}:{port} - {e}")
            return None
    ...
    def process_router(self, node: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result_data["性能评估"]["性能总结"] = "Telnet连接失败。"
            return result_data

        # 获取配置
        output = self.get_configuration_via_telnet(tn, image_type)
        if not output:
            logging.error(f"[{host}:{port}] 无法获取配置。")
            result_data["性能评估"]["性能总结"] = "无法获取配置。"
            tn.close()
            return result_data
        ...
        device_blocks = self.config_checks.get(key, {}).get("blocks", {})
        for protocol in all_protocols:
            ...
            status = device_blocks.get(block_key, {}).get("状态", "未配置")
            protocols[protocol] = status
        result_data["协议配置状态"] = protocols

        # 获取接口信息
        # 假设已经有 network_topology 数据可用，并且在主函数中进行了注释
        # 此处仅需将接口信息从注释后的拓扑结构中获取
        # 需要在主函数中传递 network_topology 给此方法
        # 为简化代码，这里暂时跳过，后续将在主函数中处理

        # 执行 NQA 测试
        # 需要 OSPF 路由的目的地址
        # 从 config_checks 中获取 OSPF 路由目的地址
        ospf_ip = None
        if "ospf 1" in device_blocks:
            # 这里假设 OSPF 配置中有某种方式获取目的地址
            # 需要根据实际配置进行调整
            # 这里暂时使用正则从配置内容中提取目的地址
            ospf_config = device_blocks["ospf 1"]["内容"]
            dest_ip_match = re.search(r'destination-address\s+ipv4\s+(\d+\.\d+\.\d+\.\d+)', ospf_config, re.IGNORECASE)
            if dest_ip_match:
                ospf_ip = dest_ip_match.group(1)
            else:
                # 如果无法从配置中提取目的地址，可以尝试从路由表中获取
                pass

        if ospf_ip:
            metrics = self.get_nqa_metrics(host, port, ospf_ip)
            if metrics:
                evaluation = self.evaluate_network_performance(metrics)
                summary = self.generate_performance_summary(evaluation)
                result_data["性能评估"]["性能指标"] = metrics
                result_data["性能评估"]["性能评价"] = evaluation
                result_data["性能评估"]["性能总结"] = summary
                self.performance_results[key] = metrics
            else:
                result_data["性能评估"]["性能总结"] = "无法获取 NQA 测试结果。"
        else:
            result_data["性能评估"]["性能总结"] = "未找到 OSPF 路由目的地址，无法进行 NQA 测试。"

        tn.close()
        return result_data

    def connect_and_process_routers(self) -> List[Dict[str, Any]]:
        """通过 Telnet 连接每个路由器，检索配置和性能评估。"""
        results = []
    ...
    def generate_summary(self, network_topology: Dict[str, Any],
                        telnet_devices: Dict[str, Any]) -> Dict[str, Any]:
        """
        生成网络摘要，包括节点、接口状态、协议配置状态和性能评估。
        输出内容为中文，并包含协议配置状态和性能评估结果。
        """
        summary = {"节点": []}
        # 创建sysname到接口信息的映射
        annotated_topology = self.annotate_network_topology(network_topology, telnet_devices)

        for result in self.connect_and_process_routers():
            sysname = result.get("sysname", "未知")
            protocols = result.get("协议配置状态", {})
            interfaces = annotated_topology.get(sysname, {}).get("接口信息", [])
            performance = result.get("性能评估", {})
            performance_summary = performance.get("性能总结", "未知")
            ...
            node_summary = {
                ...
                "接口信息": interfaces,
                "性能评估": {
                    "性能指标": performance.get("性能指标", {}),
                    "性能评价": performance.get("性能评价", {}),
...
def main(input_path: str, output_path: str, max_workers: int):
    ...
    if lab_id is not None:
        ...
        manager = NetworkManager(telnet_info, max_workers=max_workers)
        unl_content = manager.read_unl_file(lab_id)
        if unl_content:
            network_topology = manager.parse_unl_content(unl_content)
        else:
    ...
    if network_topology:
        summary = manager.generate_summary(network_topology, manager.collect_telnet_results()["telnet_devices"])
    else:
        summary = {
            "节点": []
        }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="通过 Telnet 从路由器检索配置、执行 NQA 测试并生成网络性能评估摘要。")
    parser.add_argument("-i", "--input", required=True, help="param.json的路径，使用 {t} 表示最新的文件夹编号。")
    parser.add_argument("-o", "--output", required=True,
                        help="输出路径，用于存储处理信息，使用 {t} 表示最新的文件夹编号。")
    parser.add_argument("--max-workers", type=int, default=20, help="最大并发线程数（默认为 20）。")
    args = parser.parse_args()

    main(args.input, args.output, max_workers=args.max_workers)
