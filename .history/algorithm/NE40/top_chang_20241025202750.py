...
def write_interface_status(data_txt_path: str, mapping: Dict[str, Any]):
    ...
    节点: sysname1 (host:port)
        ...
        接口: Ethernet1/0/1接口配置状态: 未配置IP地址

    节点: sysname2 (host:port)
        接口: Ethernet1/0/2接口配置状态: 已配置IP地址
        接口: Ethernet1/0/3接口配置状态: 未配置IP地址
    ...
    try:
        with open(data_txt_path, 'w', encoding='utf-8') as f:
            ...
            for connection in connections:
                ...
                # 检查 interface1 是否配置了IP
                iface1_configured = any(
                    iface['Interface'].lower() == interface1.lower() for iface in interfaces1
                )

                # 检查 interface2 是否配置了IP
                iface2_configured = any(
                    iface['Interface'].lower() == interface2.lower() for iface in interfaces2
                )

                # 如果 interface1 未配置IP，则记录
                if not iface1_configured:
                    unconfigured_connections.append(f"    {node1}: {interface1} 已连接到 {node2}: {interface2} 但未配置IP地址")

                # 如果 interface2 未配置IP，则记录
