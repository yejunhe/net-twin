...
class RouterManager:
    def __init__(self, telnet_info: Dict[str, Any], max_workers: int = 20):
        ...
        self.interface_briefs: Dict[str, Dict[str, Any]] = {}  # 新增用于存储接口简要信息
        ...
        if output:
            ...
            if sysname:
                ...
                with self.telnet_lock:
                    ...
                    # 解析接口简要信息
                    interface_brief = self.parse_interface_brief(output)
                    self.interface_briefs[key] = interface_brief
        else:
            logging.warning(f"[{tn.host}:{tn.port}] No output received from Telnet commands.")
        return output

    def parse_interface_brief(self, interface_output: str) -> Dict[str, Any]:
        """
        解析 'display ip interface brief' 的输出内容。
        提取每个接口的名称和对应的 IP 地址，忽略 IP 地址为 'unassigned' 的接口。
        """
        interface_info = {}
        ...
        pattern = re.compile(
            r'^\s*(?P<interface>\S+)\s+'
            r'(?P<ip_address>\S+)\s+'
            r'(?P<physical_state>\S+)\s+'
            r'(?P<protocol_state>\S+)\s+'
            r'(?P<vpn>\S+)$'
        )

        for line in lines:
            if not header_found:
                if re.search(r'Interface\s+IP\s+Mask\s+Physical\s+Protocol\s+VPN', line, re.IGNORECASE):
                    header_found = True
            ...
            if match:
                ...
                # mask = match.group('mask')  # 如果需要，可以提取mask
                # physical_state = match.group('physical_state')
                # protocol_state = match.group('protocol_state')
                # vpn = match.group('vpn')  # 如果需要，可以提取VPN信息

                if ip_address.lower() != 'unassigned':
                    # 只记录接口名称与IP地址的对应关系
                    interface_info[interface] = ip_address
                    logging.info(f"Interface {interface} has IP {ip_address}")
                else:
                    logging.info(f"Interface {interface} is unassigned. Skipping.")
            else:
                logging.warning(f"Unmatched line format in interface brief: {line}")

        logging.info("Interface brief parsed successfully.")
        ...
        return {
            "telnet_devices": {
                host_port: {
                    ...
                    "interface_brief": self.interface_briefs.get(host_port, {})
                }
