...
class RouterManager:
    ...
    def parse_display_bgp_all_summary(self, output: str) -> Optional[Dict[str, Any]]:
        ...
        :param output: Output of the command.
        :return: Dictionary containing BGP local router ID, local AS number,
                total number of peers, peers in established state count,
                and list of peers not in established state.
        ...
        # Patterns
        router_id_regex = re.compile(r'BGP local router ID\s*:\s*(\d+\.\d+\.\d+\.\d+)', re.IGNORECASE)
        local_as_regex = re.compile(r'Local AS number\s*:\s*(\d+)', re.IGNORECASE)
        total_peers_regex = re.compile(r'Total number of peers\s*:\s*(\d+)', re.IGNORECASE)
        established_peers_regex = re.compile(r'Peers in established state\s*:\s*(\d+)', re.IGNORECASE)

        # Flags
        in_peer_table = False

        # Updated peer entry regex for more flexibility
        ...
        for line in lines:
            ...
            # Extract Total number of peers
            if bgp_info["total_peers"] == 0:
                match = total_peers_regex.search(line)
                if match:
                    bgp_info["total_peers"] = int(match.group(1))
                    logging.debug(f"Detected Total number of BGP peers: {bgp_info['total_peers']}")
                    continue

            # Extract Peers in established state
            if bgp_info["established_peers"] == 0:
                match = established_peers_regex.search(line)
                if match:
                    bgp_info["established_peers"] = int(match.group(1))
                    logging.debug(f"Detected Peers in established state: {bgp_info['established_peers']}")
                    continue
            ...
            if in_peer_table:
                ...
                # Parse peer entries using updated regex
                ...
                if match:
                    ...
                    if state.lower() != 'established':
                        ...
                        logging.debug(f"Detected non-established BGP peer: {peer_ip}, AS: {peer_as}, State: {state}")
                else:
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        # Ensure return statement is outside the loop
        return {
            "telnet_devices": {
                host_port: {
                    "sysname": sysname,
...
def write_interface_status(data_txt_path: str, mapping: Dict[str, Any]):
    ...
    """
        ...
        节点: sysname1 (host:port)
            ...
            BGP 建立状态的邻居数量: 3
            BGP 未建立状态的邻居:
                Peer IP: 10.0.34.2, AS: 200, State: Active
        ...
        with open(data_txt_path, 'w', encoding='utf-8') as f:
            ...
            for host_port, device_info in telnet_devices.items():
                ...
                if bgp_info and bgp_info.get("bgp_local_router_id") != "未知":
                    ...
                    f.write(f"    BGP 自治域: {bgp_info.get('bgp_local_as_number')}\n")
