...
def annotate_network_topology(network_topology: Dict[str, Any],
    ...
    for node in network_topology.get("nodes", []):
        ...
        for interface in node.get("interfaces", []):
            ...
            # 组合接口类型和名称，转换为 'interface Ethernet1/0/0' 格式
            # 假设 'ethernet' 类型对应 'Ethernet'，其他类型需根据实际情况调整
            type_mapping = {
                "ethernet": "Ethernet",
                # 可以根据实际接口类型继续添加映射
            }
            interface_type_mapped = type_mapping.get(interface_type.lower(), interface_type.capitalize())
            required_block_name = f"interface {interface_type_mapped}{interface_name.replace('/', '/')}"

            # 检查该接口是否已配置
            is_configured = required_block_name in checks and checks[required_block_name]["status"] == "已配置"
