...
class TopologyBuilder:
    def __init__(self, sysname_map: Dict[str, str], neighbor_info: Dict[str, Dict[str, Any]]):
    ...
    def build_graph(self):
        for host_port, sysname in self.sysname_map.items():
            self.graph.add_node(sysname, host_port=host_port)
        ...
        for host_port, neighbors in self.neighbor_info.items():
            ...
            for protocol, neighbor_list in neighbors.items():
                for neighbor in neighbor_list:
                    ...
                    if neighbor_sysname:
                        self.graph.add_edge(sysname, neighbor_sysname, protocol=protocol)
    ...
    def visualize_topology(self, output_path: str):
        try:
            ...
            logging.error("需要安装 matplotlib 库以可视化拓扑图。")
...
class TopologyValidator:
    ...
    def detect_cycles(self):
        try:
            cycles = list(nx.find_cycle(self.graph, orientation='ignore'))
            if cycles:
    ...
    def verify_protocol_consistency(self):
        for edge in self.graph.edges(data=True):
            protocols = edge[2].get('protocol', [])
            if isinstance(protocols, str):
                protocols = [protocols]
            if len(protocols) < 2:
                self.errors.append(f"连接 {edge[0]} <--> {edge[1]} 的协议不一致或不完整。")
                logging.warning(f"连接 {edge[0]} <--> {edge[1]} 的协议不一致或不完整。")
...
class IsisFaultDetector:
    ...
    def parse_display_isis_brief(self, output: str, host_port: str):
        ...
        # 使用正则表达式精确匹配字段和布尔值
        l1_overflow_match = re.search(r'L1 Lsp Over Flow:\s*(\w+)', output, re.IGNORECASE)
        if l1_overflow_match:
            value = l1_overflow_match.group(1).strip().lower()
            self.fault_info[host_port]["l1_lsp_overflow"] = value == 'true'
            logging.debug(f"[{host_port}] L1 Lsp Over Flow: {value}")

        l2_overflow_match = re.search(r'L2 Lsp Over Flow:\s*(\w+)', output, re.IGNORECASE)
        if l2_overflow_match:
            value = l2_overflow_match.group(1).strip().lower()
            self.fault_info[host_port]["l2_lsp_overflow"] = value == 'true'
            logging.debug(f"[{host_port}] L2 Lsp Over Flow: {value}")

        level1_loop_match = re.search(r'Level-1 Avoid Redistribute Loop Capability:\s*(\w+)', output, re.IGNORECASE)
        if level1_loop_match:
            value = level1_loop_match.group(1).strip().lower()
            self.fault_info[host_port]["level1_avoid_redistribute_loop"] = value == 'true'
            logging.debug(f"[{host_port}] Level-1 Avoid Redistribute Loop Capability: {value}")

        level2_loop_match = re.search(r'Level-2 Avoid Redistribute Loop Capability:\s*(\w+)', output, re.IGNORECASE)
        if level2_loop_match:
            value = level2_loop_match.group(1).strip().lower()
            self.fault_info[host_port]["level2_avoid_redistribute_loop"] = value == 'true'
            logging.debug(f"[{host_port}] Level-2 Avoid Redistribute Loop Capability: {value}")

        logging.info(f"[{host_port}] Parsed ISIS brief information: {self.fault_info[host_port]}")
    ...
    def detect_faults(self, tn: telnetlib.Telnet, host_port: str):
        # 已在 parse_display_isis_brief 中初始化 self.fault_info[host_port]
        ...
        if info["l2_lsp_overflow"]:
            ...
            self.faults.append((host_port, fault))
            logging.warning(fault)
        # 仅在 avoid_redistribute_loop 为 True 时记录故障
...
class RouterManager:
    ...
    def execute_telnet_commands(self, tn: telnetlib.Telnet, commands: List[str], host_port: str) -> Dict[str, str]:
        """Executes a list of commands on a Telnet session and retrieves the output."""
        output = {}
        try:
            tn.write(b'\n')
            time.sleep(1)
            ...
            for cmd in commands:
                ...
                logging.info(f"[{host_port}] Sending command: {cmd}")
                time.sleep(1)
                cmd_output = tn.read_very_eager().decode('ascii', errors='ignore')
                output[cmd] = cmd_output
                logging.debug(f"[{host_port}] Output for '{cmd}':\n{cmd_output}")
    ...
    def get_configuration_via_telnet(self, tn: telnetlib.Telnet, image_type: str, host_port: str):
        """Executes all necessary commands based on the device's image_type."""
...
def main(input_path: str, output_path: str):
    ...
    # 拓扑构建与验证
    topology_builder = TopologyBuilder(sysname_map, router_manager.telnet_info)
    topology_builder.build_graph()
    topology_image_path = os.path.join(os.path.dirname(output_path), 'topology.png')
