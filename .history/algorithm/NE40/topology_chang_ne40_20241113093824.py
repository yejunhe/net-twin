...
class RouterManager:
    ...
    def format_interface_name(self, iface_name: str, iface_type: str) -> str:
        """
        格式化接口名称，将不同格式的接口名统一转换为标准格式。
        支持 'e1/0/0' 和 'Ethernet 1/0/0' 两种格式。

        :param iface_name: 原始接口名称。
        :param iface_type: 接口类型（如 'ethernet'）。
        :return: 格式化后的接口名称，例如 'Ethernet1/0/0'。
        """
        original_iface_name = iface_name  # Preserve the original for logging
        iface_name = iface_name.strip()
        iface_formatted = ""

        if iface_name.lower().startswith('e') and not iface_name.lower().startswith('ethernet'):
            # 处理 'e1/0/0' 格式
            iface_number = iface_name[1:]  # 移除 'e' 前缀
            iface_formatted = f"Ethernet{iface_number}"
        elif iface_name.lower().startswith('ethernet'):
            # 处理 'Ethernet 1/0/0' 或 'Ethernet1/0/0' 格式
            # 使用正则表达式提取数字部分
            match = re.match(r'ethernet\s*(\d+/\d+/\d+)', iface_name, re.IGNORECASE)
            if match:
                iface_formatted = f"Ethernet{match.group(1)}"
            else:
                # 如果不匹配预期格式，按类型格式化并 remove any spaces
                iface_formatted = f"{iface_type.capitalize()}{iface_name.replace(' ', '')}"
        else:
            # 其他格式，按类型格式化
            iface_formatted = f"{iface_type.capitalize()}{iface_name}"

        logging.debug(f"格式化接口名称: 原始='{original_iface_name}' -> 格式化='{iface_formatted}'")
        return iface_formatted
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            ...
            for iface in node_interfaces:
                # 格式化接口名称，例如 type="ethernet" name="e1/0/0" 或 "Ethernet 1/0/0" => "Ethernet1/0/0"
                iface_name = iface['name']
                iface_type = iface.get('type', 'ethernet')  # Default to 'ethernet' if not specified
                iface_formatted = self.format_interface_name(iface_name, iface_type)
                ...
                logging.info(f"[{host_port}] {status.strip()}")
    ...
    def write_interface_status(self):
        ...
        try:
            with open(self.data_txt_path, 'w', encoding='utf-8') as f:
                interface_status = self.mapping.get("interface_status", {})
                for host_port, info in interface_status.items():
                    sysname = info.get("sysname", "未知节点")
            ...
            if 'modified_nodes' in differences:
                ...
                for node_change in differences['modified_nodes']:
                    ...
                    for key, change in changes.items():
                        if key == 'interfaces':
                            added_interfaces = self.get_added_interfaces(change.get('old', []), change.get('new', []))
                            removed_interfaces = self.get_removed_interfaces(change.get('old', []), change.get('new', []))
                            modified_interfaces = self.get_modified_interfaces(change.get('old', []), change.get('new', []))
                            if added_interfaces:
                                ...
                                for iface in added_interfaces:
                                    ...
                                    network_id = iface.get('network_id', 'N/A')
                                    report_lines.append(f"     - ID: {iface_id}, Name: {iface_name}, Type: {iface_type}, Network ID: {network_id}")
...
class CombinedScriptManager:
    ...
    def run(self):
        ...
        interface_mapping = router_manager.collect_results()

        # 写入接口状态到 data.txt
        output_dir = os.path.dirname(self.output_path)
        data_txt_path = os.path.join(output_dir, "data.txt")
        interface_writer = InterfaceStatusWriter(data_txt_path, interface_mapping)
