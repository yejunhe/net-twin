...
class RouterManager:
    ...
    def parse_display_ip_interface_brief(self, output: str) -> List[Dict[str, str]]:
        ...
        interface_regex = re.compile(
            ...
            r'(?P<physical>\*?down|!down|\^down|\(l\)|\(s\)|\(d\)|\(E\)|up|down)\s+'
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        interface_status = {}
        # Initialize mapping from sysname to host_port for easy lookup
        sysname_to_host_port = {sysname: host_port for host_port, sysname in self.telnet_sysnames.items()}
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            ...
            for iface in node_interfaces:
                ...
                if iface_name.lower().startswith('e'):
                    ...
                    iface_formatted = f"{iface['type'].capitalize()}{iface_name}"
                ...
                # Additionally, include Physical and Protocol status
                # Find the interface info in telnet_configurations
                interface_info = next((iface_dict for iface_dict in telnet_interfaces if iface_dict['Interface'].lower() == iface_formatted_lower), None)
                if interface_info:
                    physical_status = interface_info['Physical']
                    protocol_status = interface_info['Protocol']
                else:
                    physical_status = "未知"
                    protocol_status = "未知"

                status = f"{iface_formatted}接口配置状态: {config_status}, Physical: {physical_status}, Protocol: {protocol_status}"
...
def write_interface_status(data_txt_path: str, mapping: Dict[str, Any]):
    """
    将接口状态和未配置IP地址的连接写入 data.txt，格式如下：
    节点: sysname1 (host:port)
        接口: Ethernet1/0/0接口配置状态: 已配置IP地址, Physical: *down, Protocol: down
        接口: Ethernet1/0/1接口配置状态: 已配置IP地址, Physical: up, Protocol: up

    节点: sysname2 (host:port)
        接口: Ethernet1/0/2接口配置状态: 未配置IP地址, Physical: up, Protocol: down

    未配置IP地址的连接:
        sysname1: Ethernet1/0/1 已连接到 sysname2: Ethernet1/0/3 但未配置IP地址
    """
    try:
        with open(data_txt_path, 'w', encoding='utf-8') as f:
            ...
            # 构建 sysname 到 host_port 的映射
            sysname_to_host_port = {sysname: host_port for host_port, sysname in mapping["telnet_devices"].items()}

            # 收集未配置IP地址的连接信息
            connections = mapping.get("network_connections", [])
            unconfigured_connections = []
            ...
            for connection in connections:
                ...
                host_port1 = sysname_to_host_port.get(node1)
                host_port2 = sysname_to_host_port.get(node2)

                if not host_port1 or not host_port2:
                    logging.warning(f"无法找到节点 {node1} 或 {node2} 的 host_port。跳过此连接。")
                    continue

                # 获取接口配置
                interfaces1 = telnet_devices.get(host_port1, {}).get("interfaces", [])
                interfaces2 = telnet_devices.get(host_port2, {}).get("interfaces", [])

                # Check if interface1 has IP configured
                iface1_info = next((iface for iface in interfaces1 if iface['Interface'].lower() == interface1.lower()), None)
                iface1_configured = False
                if iface1_info and iface1_info['IP Address/Mask'].lower() != 'unassigned':
                    iface1_configured = True

                # Check if interface2 has IP configured
                iface2_info = next((iface for iface in interfaces2 if iface['Interface'].lower() == interface2.lower()), None)
                iface2_configured = False
                if iface2_info and iface2_info['IP Address/Mask'].lower() != 'unassigned':
                    iface2_configured = True

                # If interface1 is not configured, record
                if not iface1_configured:
                    unconfigured_connections.append(f"    {node1}: {interface1} 已连接到 {node2}: {interface2} 但未配置IP地址")

                # If interface2 is not configured, record
                if not iface2_configured:
                    unconfigured_connections.append(f"    {node2}: {interface2} 已连接到 {node1}: {interface1} 但未配置IP地址")

            # 写入未配置IP地址的连接信息
            if unconfigured_connections:
                f.write("未配置IP地址的连接:\n")
                for conn in unconfigured_connections:
                    f.write(f"{conn}\n")
            else:
                f.write("未配置IP地址的连接: 无\n")

        logging.info(f"接口状态和未配置IP地址的连接已写入 {data_txt_path}")
    except IOError as e:
