...
# 配置日志记录，便于跟踪和控制
logging.basicConfig(
    level=logging.DEBUG,  # 设置为 DEBUG 以获取更详细的日志
...
class RouterManager:
            ...
            if sysname:
                ...
                with self.telnet_lock:
                    ...
                    if 'display ip interface brief' in command_outputs:
                        ...
                        logging.warning(f"[{tn.host}:{tn.port}] 缺少 'display ip interface brief' 的输出。")
                    ...
                    if 'display ospf interface' in command_outputs:
                        ...
                        logging.warning(f"[{tn.host}:{tn.port}] 缺少 'display ospf interface' 的输出。")
                    ...
                    if 'display ospf peer' in command_outputs:
                        ...
                        logging.warning(f"[{tn.host}:{tn.port}] 缺少 'display ospf peer' 的输出。")
                    ...
                    if 'display isis interface' in command_outputs:
                        ...
                        logging.warning(f"[{tn.host}:{tn.port}] 缺少 'display isis interface' 的输出。")
                    ...
                    if 'display isis peer' in command_outputs:
                        ...
                        logging.warning(f"[{tn.host}:{tn.port}] 缺少 'display isis peer' 的输出。")
                    ...
                    if 'display bgp all summary' in command_outputs:
                        ...
                        logging.warning(f"[{tn.host}:{tn.port}] 缺少 'display bgp all summary' 的输出。")
        else:
            logging.warning(f"[{tn.host}:{tn.port}] Telnet 命令未收到输出。")
        return command_outputs

    def parse_display_ospf_peer(self, output: str) -> Tuple[Optional[str], Optional[List[str]]]:
        """
        解析 'display ospf peer' 的输出，提取当前节点的 Router ID 和邻居 Router IDs。

        :param output: 命令输出。
        :return: 包含 Router ID 和邻居 Router IDs 的元组。如果没有相关输出，返回 (None, None)。
        """
        router_id = None
        neighbors = []
        lines = output.splitlines()
        logging.debug(f"解析 'display ospf peer' 输出。")
        ...
        for line in lines:
            ...
            if ospf_process_match:
                ...
                logging.debug(f"检测到 Router ID: {router_id}")
                continue
            ...
            if router_id_found:
                ...
                if neighbor_match:
                    ...
                    logging.debug(f"检测到邻居 Router ID: {neighbor_id}")

        if router_id_found:
            logging.info(f"提取的 Router ID: {router_id} 及邻居: {neighbors}")
            return router_id, neighbors
        else:
            logging.info("在 'display ospf peer' 输出中未找到 OSPF 进程信息。")
            return None, None

    def parse_display_isis_interface(self, output: str) -> List[str]:
        """
        解析 'display isis interface' 的输出，提取配置了 ISIS 的接口。

        :param output: 命令输出。
        :return: 配置了 ISIS 的接口名称列表，标准化格式。
        """
        interfaces = []
        lines = output.splitlines()
        logging.debug("解析 'display isis interface' 输出。")
        ...
        for line in lines:
            ...
            if iface.lower().startswith('eth'):
                iface_number = iface[3:]  # 移除 'Eth' 前缀
                iface_formatted = f"Ethernet{iface_number}"
            else:
                iface_formatted = iface.capitalize()  # 例如 'Loop0' 保持为 'Loop0'
            interfaces.append(iface_formatted)
            logging.debug(f"检测到 ISIS 配置的接口: {iface_formatted}")
        
        logging.debug(f"解析的 ISIS 接口: {interfaces}")
        return interfaces

    def parse_display_isis_peer(self, output: str) -> Optional[int]:
        """
        解析 'display isis peer' 的输出，提取 ISIS 邻居的总数。

        :param output: 命令输出。
        :return: 邻居数量。如果无法解析，返回 None。
        """
        total_peers = None
        lines = output.splitlines()
        ...
        for line in lines:
            ...
            if match:
                ...
                logging.debug(f"检测到总 ISIS 邻居数: {total_peers}")
                break

        if total_peers is not None:
            logging.info(f"提取的总 ISIS 邻居数: {total_peers}")
            return total_peers
        else:
            logging.info("在 'display isis peer' 输出中未找到 'Total Peer(s):' 行。")
            return None
        
    def parse_display_bgp_all_summary(self, output: str) -> Optional[Dict[str, Any]]:
        """
        解析 'display bgp all summary' 的输出，提取 BGP 信息。

        :param output: 命令输出。
        :return: 包含 BGP 信息的字典。如果解析失败，返回 None。
        """
        bgp_info = {
        ...
        in_peer_table = False  # 标志是否在解析邻居表
        ...
        for line in lines:
            ...
            for key, value in key_value_matches:
                ...
                if key == 'bgp local router id':
                    ...
                    logging.debug(f"检测到总 BGP 邻居数: {bgp_info['bgp_total_peers']}")
                elif key == 'peers in established state':
                    bgp_info["bgp_established_peers"] = int(value)
                    logging.debug(f"检测到已建立的 BGP 邻居数: {bgp_info['bgp_established_peers']}")
            ...
            if in_peer_table:
                ...
                if match:
                    ...
                    if state != 'established':
                        ...
                        logging.debug(f"检测到未建立的 BGP 邻居: IP={match.group('peer_ip')}, AS={match.group('peer_as')}, 状态={match.group('state').capitalize()}")
                else:
                    logging.debug(f"未匹配的 BGP 邻居行: {line}")
    ...
    def parse_display_ip_interface_brief(self, output: str) -> List[Dict[str, str]]:
        """
        解析 'display ip interface brief' 的输出，排除未分配 IP 的接口，并返回结构化数据。
        ...
        for line in lines:
            ...
            if not header_found:
                ...
                if match:
                    ...
                    logging.debug(f"未匹配的 'display ip interface brief' 行: {line}")
                    continue

        logging.debug(f"解析的 Telnet 接口: {interfaces}")
        return interfaces

    def parse_display_ospf_interface(self, output: str) -> List[str]:
        """
        解析 'display ospf interface' 的输出，提取配置了 OSPF 的接口。

        :param output: 命令输出。
        :return: 配置了 OSPF 的接口名称列表，标准化格式。
        """
        interfaces = []
        lines = output.splitlines()
        parsing = False  # 标志是否开始解析
        ...
        for line in lines:
            ...
            if parsing:
                ...
                if match:
                    ...
                    logging.debug(f"找到 OSPF 接口: {iface}")
                    # 格式化接口名称
                    if iface.lower().startswith('eth'):
                        iface_number = iface[3:]  # 移除 'Eth' 前缀
                        iface_formatted = f"Ethernet{iface_number}"
                        interfaces.append(iface_formatted)
                        logging.debug(f"格式化后的 OSPF 接口名称: {iface_formatted}")
            ...
            for node in nodes:
                ...
                if "huaweine40" in image_type:
                    ...
                    if not host or not port:
                        logging.warning(f"节点的 host IP 或端口缺失（image_type='{image_type}'）。跳过。")
                        continue
                    try:
                        ...
                        logging.error(f"通过 Telnet 连接到 {host}:{port} 失败: {e}")
    ...
    def collect_results(self) -> Dict[str, Any]:
        """
        收集所有结果，进行接口匹配，并输出接口配置状态，包括 OSPF、ISIS 和 BGP 状态。
        """
        # 存储接口状态信息
        interface_status = {}
        ospf_status = {}
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            ...
            logging.debug(f"[{host_port}] 配置了 OSPF 的接口: {ospf_interfaces_lower}")
            logging.debug(f"[{host_port}] 配置了 ISIS 的接口: {isis_interfaces_lower}")
            if bgp_info:
        ...
        return {
            "telnet_devices": {
                host_port: {
                    ...
                    # 添加与 NQA 相关的信息
                    "ospf_routes": self.ospf_routes.get(host_port, {}),
    ...
    def parse_unl_file(self, unl_content: str):
        ...
        try:
            ...
            # 现在，对于每个 network_id，如果有且仅有两个接口，则创建连接
            for network_id, interfaces in network_to_interfaces.items():
                if len(interfaces) == 2:
                    ...
                    logging.info(f"连接 {connection['node1']}:{connection['interface1']} <-> {connection['node2']}:{connection['interface2']} 通过 network_id {network_id}")
                else:
                    logging.warning(f"network_id {network_id} 不包含恰好两个接口。跳过连接。")

            self.network_connections = connections
            logging.info("成功将 UNL 文件解析为网络连接。")
        except ET.ParseError as e:
            logging.error(f"解析 UNL 文件时出错: {e}")

    def integrate_nqa_tests(self):
        """
        集成 NQA 测试到现有的 RouterManager 类中。
        """
        # 您需要在此处集成原始代码中的 NQA 相关方法
        # 以下是从原始代码中提取的相关方法
        ...
        def parse_routing_table(self, routing_table: str) -> Optional[str]:
            ...
            :return: 符合条件的 OSPF 目的 IP 地址或 None。
            """
            current_dest_ip = None  # 当前的 Destination/Mask
                    ...
                    if current_dest_ip:
                        try:
                            ...
                            if ip.packed[-1] != 0:  # 检查最后一个字节是否非零
                                logging.info(f"找到最后一个八位组非零的 OSPF 路由目的地址: {current_dest_ip}")
                                return current_dest_ip
                            else:
                                logging.debug(f"跳过最后一个八位组为零的 OSPF 路由目的地址: {current_dest_ip}")
                        except ValueError:
                            logging.warning(f"无效的 IP 地址格式: {current_dest_ip}")
                            continue
                    else:
                        logging.warning(f"在路由表行中未找到有效的 Destination/Mask: {line}")

            logging.info("未找到符合条件的 OSPF 路由目的地址")
            return None
            ...
            try:
                ...
                while attempt_count < max_attempts:
                    ...
                    if "The test is finished" in partial_output:
                        logging.info(f"NQA 测试完成 {dest_ip}，尝试次数: {attempt_count + 1}")
                        break

                    attempt_count += 1
                    logging.info(f"尝试 {attempt_count}/{max_attempts} 获取 {dest_ip} 的 NQA 结果...")

                    if attempt_count >= max_attempts:
                        logging.warning(f"已达到最大尝试次数 {max_attempts}，{dest_ip} 的测试结果可能不完整。")
                        break

                # 记录并返回结果
                logging.info(f"{dest_ip} 的 NQA 测试结果:")
                logging.info(result)
                ...
                if metrics is None:
                    logging.warning(f"未能解析 {dest_ip} 的 NQA 结果。")
                return metrics

            except Exception as e:
                logging.error(f"NQA 测试期间发生错误: {e}")
                return None
        ...
        # 方法：通过 Telnet 连接每个路由器，检索 sysname、OSPF 路由，执行 NQA 测试，并评估网络性能
    ...
    def load_telnet_info(input_path: str) -> Dict[str, Any]:
        try:
            ...
            logging.error(f"param.json 文件未在路径找到: {input_path}")
            sys.exit(1)
    ...
    def write_interface_status(self, data_txt_path: str, mapping: Dict[str, Any]):
        """
        将接口状态和 OSPF/ISIS/BGP 状态写入 data.txt，格式如下：
        ...
        try:
            with open(data_txt_path, 'w', encoding='utf-8') as f:
                ...
                for host_port, device_info in telnet_devices.items():
                    ...
                    performance_metrics = device_info.get("performance_metrics", {})
                    ...
                    if performance_metrics:
                        latency = performance_metrics.get("latency", "未知")
                        jitter = performance_metrics.get("jitter", "未知")
                        packet_loss = performance_metrics.get("packet_loss", "未知")
                        performance_summary = device_info.get("performance_summary", "无性能评估结果。")

                        f.write(f"    NQA 性能指标:\n")
                        f.write(f"        延迟 (平均 RTT): {latency} ms\n")
                        f.write(f"        抖动 (平均抖动): {jitter} ms\n")
                        f.write(f"        丢包率: {packet_loss} %\n")
                        f.write(f"    性能评估总结: {performance_summary}\n")
                    else:
                        f.write(f"    NQA 性能指标: 无\n")
                        f.write(f"    性能评估总结: 无性能评估结果。\n")
    ...
    def main(input_path: str, output_path: str, max_threads: int = 10):
        ...
        if lab_id is not None:
            ...
            logging.warning("param.json 中未找到 labId。")
        ...
        # 执行 NQA 测试并评估网络性能
        performance_results = router_manager.connect_and_get_sysnames_routes_and_nqa()
        ...
        for result in performance_results:
            ...
            key = f"{host}:{port}"
            if key in mapping["telnet_devices"]:
                mapping["telnet_devices"][key]["performance_metrics"] = result.get("nqa_result", {})
                mapping["telnet_devices"][key]["performance_summary"] = result.get("performance_summary", "无性能评估结果。")

        logging.info("收集到的路由器配置:")
        logging.info(json.dumps(mapping, indent=4, ensure_ascii=False))
        write_output(output_path, mapping)

        # 定义 data.txt 的路径，放在 output_path 相同的目录中
        output_dir = os.path.dirname(output_path)
        data_txt_path = os.path.join(output_dir, "data.txt")
        write_interface_status(data_txt_path, mapping)

    if __name__ == "__main__":
        parser = argparse.ArgumentParser(description="处理 param.json 中的路由器配置并执行 NQA 测试。")
