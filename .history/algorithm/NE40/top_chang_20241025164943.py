        ...
        for host_port, neighbors in self.neighbor_info.items():
            ...
            for protocol, neighbor_list in neighbors.items():
                for neighbor in neighbor_list:
                    # 假设邻居的标识是sysname或host_port
                    # 需要将neighbor转换为sysname
...
class RouterManager:
    ...
    def get_neighbor_info_from_telnet(self, tn: telnetlib.Telnet, host_port: str) -> Dict[str, List[str]]:
        """从Telnet会话中解析邻居信息。"""
        neighbors = {
            "OSPF": [],
            "ISIS": [],
        ...
        # 解析OSPF邻居
        ospf_peer_output = self.ospf_outputs.get(host_port, {}).get('display ospf peer', '')
        for line in ospf_peer_output.splitlines():
            match = re.search(r'Neighbor\s+(\S+)', line)
            if match:
                neighbor = match.group(1)
                neighbors["OSPF"].append(neighbor)

        # 解析ISIS邻居
        isis_peer_output = self.isis_outputs.get(host_port, {}).get('display isis peer', '')
        for line in isis_peer_output.splitlines():
            match = re.search(r'Peer\s+(\S+)', line)
            if match:
                neighbor = match.group(1)
                neighbors["ISIS"].append(neighbor)

        # 解析BGP邻居
        bgp_output = self.bgp_outputs.get(host_port, '')
        # 假设BGP输出中包含邻居信息，可以根据具体设备的输出格式调整
        # 这里以假设的格式为例
        for line in bgp_output.splitlines():
            match = re.search(r'BGP Neighbor:\s+(\S+)', line)
            if match:
                neighbor = match.group(1)
                neighbors["BGP"].append(neighbor)

        logging.info(f"[{host_port}] 解析到的邻居信息: {neighbors}")
        return neighbors
...
def main(input_path: str, output_path: str):
    ...
    # 拓扑构建与验证
    neighbor_info = {}
    for host_port in sysname_map:
        neighbors = router_manager.get_neighbor_info_from_telnet(None, host_port)  # tn 参数不再需要
        neighbor_info[host_port] = neighbors
