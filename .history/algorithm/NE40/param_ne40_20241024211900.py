...
class RouterManager:
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            ...
            for iface in node_interfaces:
                ...
                if peers:
                    neighbor_info = "; ".join([f"Router ID: {peer['router_id']}, Address: {peer['address']}" for peer in peers])
                else:
                    neighbor_info = "无邻居"
    ...
    def parse_display_ospf_peer(self, output: str) -> List[Dict[str, Any]]:
        ...
        # 正则表达式匹配状态行
        state_regex = re.compile(r'State:\s+(?P<state>\S+)\s+Mode:Nbr is Slave\s+Priority:\s+(?P<priority>\d+)', re.IGNORECASE)

        for line in lines:
            # 寻找接口的邻居信息开始
            match_interface = interface_regex.match(line)
            if match_interface:
                current_interface = match_interface.group('interface')
            ...
            if parsing:
                ...
                # 可以根据需要提取更多信息，如 State, Priority 等
                match_state = state_regex.match(line)
                if match_state and current_interface:
                    # 找到对应的邻居，更新状态信息
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            ...
            for iface in node_interfaces:
                ...
                peers = [peer for peer in ospf_peers if peer['interface'].lower() == iface_formatted_lower]
                if peers:
                    neighbor_info = "; ".join([f"Router ID: {peer.get('router_id', '未知')}, Address: {peer.get('address', '未知')}, State: {peer.get('state', '未知')}, Priority: {peer.get('priority', '未知')}" for peer in peers])
                else:
                    neighbor_info = "无邻居"
    ...
    def write_interface_status(self, data_txt_path: str, mapping: Dict[str, Any]):
        ...
        节点: sysname1 (host:port)
            ...
            接口: Ethernet1/0/1接口配置状态: 已配置, OSPF已配置, 邻居: Router ID: 2.2.2.2, Address: 10.0.23.1, State: Full, Priority: 1
