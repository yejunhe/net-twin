...
class RouterManager:
    def __init__(self, telnet_info: Dict[str, Any], max_workers: int = 20):
        ...
        # 定义需要检查的配置块及其对应的检查模式
        # 如果值为None，则只检查配置块的存在性
        self.required_blocks = {
            'sysname': None,  # 不需要特定模式，只需存在
            'bgp': None,
            'ospf': None,
            'isis': None,
            'interface Ethernet1/0/0': ['undo shutdown', 'ip address', 'ospf enable'],
            'interface Ethernet1/0/0.192': ['vlan-type dot1q', 'ip address', 'mac-address'],
            'interface Ethernet1/0/1': ['undo shutdown', 'ip address', 'ospf enable'],
            'interface Ethernet1/0/2': ['undo shutdown', 'ip address', 'ospf enable'],
            'interface LoopBack0': ['ip address', 'ospf enable'],
            'interface NULL0': None  # 不需要特定模式，只需存在
    ...
    def check_required_blocks(self, cleaned_config: str) -> Dict[str, str]:
        ...
        对于每个配置块，部分匹配保留块中的内容即可标记为“已配置”。
        返回一个字典，键为配置块名称，值为状态（'已配置' 或 '未配置'）。
        """
        checks = {}
        # 将清理后的配置块分割为列表
            ...
            if block.startswith('interface '):
                ...
                if block in cleaned_blocks_content:
                    if patterns:
                        ...
                        # 如果没有指定检查模式，只要接口块存在即为已配置
                        checks[block] = "已配置"
                else:
                    checks[block] = "缺少配置"
            elif block in ['sysname', 'bgp', 'ospf', 'isis']:
                # 对于这些配置块，只需检查其是否存在且有内容
                if block in cleaned_blocks_content:
                    block_content = cleaned_blocks_content[block]
                    # 检查配置块是否有除第一行之外的内容
                    if len(block_content.splitlines()) > 1:
                        checks[block] = "已配置"
                    else:
                        # 对于 'sysname', 即使只有一行也算已配置
                        if block == 'sysname':
                            checks[block] = "已配置"
                        else:
                            checks[block] = "缺少配置"
                ...
                if block in cleaned_blocks_content:
                    if patterns:
                        ...
                        # 如果没有指定检查模式，只要配置块存在即为已配置
                        checks[block] = "已配置"
                else:
                    checks[block] = "缺少配置"

        # 修改检查结果，对于 'sysname', 'bgp', 'ospf', 'isis' 使用 "未配置" 代替 "缺少配置"
        for block in ['sysname', 'bgp', 'ospf', 'isis']:
            if block in checks and checks[block] == "缺少配置":
                checks[block] = "未配置"
