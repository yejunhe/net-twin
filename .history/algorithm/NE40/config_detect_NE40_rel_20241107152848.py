...
class RouterManager:
    ...
    def execute_telnet_commands(self, tn: telnetlib.Telnet, commands: List[str], quit_cmd: bytes) -> Dict[str, str]:
        ...
        """
        try:
            # Send an initial newline to ensure we're at the prompt
            tn.write(b'\n')
            time.sleep(1)
            ...
            # Modify the commands list based on the prompt format
            if prompt:
                prompt = prompt.strip()
                if prompt.startswith('[') and prompt.endswith(']'):
                    # If prompt is within [], send 'q' before other commands
                    modified_commands = ['q'] + commands
                    logging.info(f"[{tn.host}:{tn.port}] Prompt is '{prompt}', inserting 'q' before commands.")
                elif prompt.startswith('<') and prompt.endswith('>'):
                    # If prompt is within <>, proceed with commands as-is
                    modified_commands = commands
                    logging.info(f"[{tn.host}:{tn.port}] Prompt is '{prompt}', proceeding with commands.")
                else:
                    # If prompt format is unrecognized, proceed with commands and log a warning
                    modified_commands = commands
                    logging.warning(f"[{tn.host}:{tn.port}] Unrecognized prompt format '{prompt}'. Proceeding with default commands.")
            else:
                # If unable to detect prompt, proceed with commands and log a warning
                modified_commands = commands
                logging.warning(f"[{tn.host}:{tn.port}] Unable to detect prompt. Proceeding with default commands.")

            command_outputs = {}

            # Execute each command in the modified_commands list
            for cmd in modified_commands:
                tn.write(cmd.encode('ascii') + b'\n')
                logging.info(f"[{tn.host}:{tn.port}] Sending command: {cmd}")
                time.sleep(2)  # Wait to ensure complete command output
            ...
            # After executing all commands, determine if we need to send the quit command
            prompt_after_commands = self.get_prompt(tn)
            logging.debug(f"[{tn.host}:{tn.port}] Prompt after commands: {prompt_after_commands}")
