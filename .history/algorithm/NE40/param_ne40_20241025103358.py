...
class RouterManager:
    ...
    def parse_display_isis_peer(self, output: str) -> Optional[int]:
        ...
        :param output: Output of the command.
        :return: Number of ISIS peers. Returns None if not found.
        """
        total_peers = None
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            ...
            logging.debug(f"[{host_port}] ISIS Peer Count: {isis_peer_count}")
            ...
            if isis_peer_count is not None:
                if isis_peer_count > 0:
                    isis_status[host_port] = f"ISIS 配置正常，邻居数量: {isis_peer_count}"
                else:
                    if isis_interfaces:
                ...
                if isis_interfaces:
                    isis_status[host_port] = "ISIS 配置问题：未检测到邻居数量"
                else:
                    isis_status[host_port] = "ISIS 未配置"
        ...
        return {
            "telnet_devices": {
                host_port: {
                    ...
                    "ospf_interfaces": self.telnet_ospf_interfaces.get(host_port, []),
                    "isis_interfaces": self.telnet_isis_interfaces.get(host_port, []),
                    "isis_peer_count": self.telnet_isis_peer_counts.get(host_port, 0),
