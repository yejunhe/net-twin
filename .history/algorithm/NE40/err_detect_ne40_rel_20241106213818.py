...
class RouterManager:
    ...
    def execute_tracert(self, tn: telnetlib.Telnet, dest_ip: str) -> Tuple[Optional[str], str]:
        ...
        try:
            ...
            timeout = 30  # Maximum wait time for tracert to complete
            ...
            while True:
                ...
                if data:
                    tracert_output += data
                    logging.debug(f"[{tn.host}:{tn.port}] Tracert Output:\n{data}")
                    # Check for '*' in the latest lines
                    if '*' in data:
                ...
                # Check if tracert has completed by detecting the prompt
                prompt = self.get_prompt(tn)
                if prompt:
                    if tracert_status == "Tracert 执行中...":
                        # If tracert completed without '*' detection
                        tracert_status = self.parse_tracert_output(tracert_output, dest_ip)
                    break
