...
class RouterManager:
        ...
        if command_outputs:
            ...
            if sysname:
                ...
                with self.telnet_lock:
                    ...
                    self.nqa_executor.submit(self.perform_nqa_tests, key, tn)
        else:
            logging.warning(f"[{tn.host}:{tn.port}] No output received from Telnet commands.")
        return command_outputs

    def perform_nqa_tests(self, host_port: str, tn: telnetlib.Telnet):
        """
        ...
        # Assuming ospf_routes have been populated elsewhere
        ospf_ip = self.ospf_routes.get(host_port, {}).get("ospf_ip")
        if not ospf_ip:
        ...
        try:
            ...
            nqa_commands = [
                ...
                f'destination-address ipv4 {ospf_ip}\n'.encode('ascii'),
                b'probe-count 2\n',
            ...
            while attempt_count < max_attempts:
                ...
                if "The test is finished" in partial_output:
                    logging.info(f"[{host_port}] NQA test finished on attempt {attempt_count + 1}")
                    break

                attempt_count += 1
                logging.info(f"[{host_port}] Attempt {attempt_count}/{max_attempts} for NQA result on {ospf_ip}...")

                if attempt_count >= max_attempts:
                    logging.warning(f"[{host_port}] Max attempts reached for NQA on {ospf_ip}. Test result may be incomplete.")
                    break

            # 记录并返回结果
            logging.info(f"[{host_port}] NQA Test Result for {ospf_ip}:\n{result}")
            ...
            if metrics:
                ...
                with self.telnet_lock:
                    self.performance_metrics[host_port] = metrics
                    self.performance_summaries[host_port] = summary
                logging.info(f"[{host_port}] Performance evaluation: {summary}")
            else:
                logging.warning(f"[{host_port}] Failed to parse NQA results for {ospf_ip}.")
        except Exception as e:
            logging.error(f"[{host_port}] Error during NQA test - {e}")
        ...
        for line in nqa_result.splitlines():
            ...
            if jitter_match:
                try:
                    ...
                    logging.debug(f"Parsed jitter (Average): {jitter_value} ms")
                except ValueError as e:
            ...
            if packet_loss_match:
                try:
                    ...
                    logging.debug(f"Parsed packet loss: {packet_loss} %")
                except ValueError as e:
                    logging.error(f"Failed to parse packet loss from line: '{line}'. Error: {e}")
                    metrics["packet_loss"] = None

        # 如果所有指标都无法解析，记录原始 NQA 结果以便调试
        if all(value is None for value in metrics.values()):
            logging.warning("All performance metrics are None. Original NQA result:")
    ...
    def process_router(self, node: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sysname = self.telnet_sysnames.get(f"{host}:{port}")
        if sysname:
            result_data["sysname"] = sysname

            # Get OSPF route (assuming it's stored in ospf_routes)
            ospf_ip = self.ospf_routes.get(f"{host}:{port}", {}).get("ospf_ip")
            if ospf_ip:
                result_data["ospf_ip"] = ospf_ip

                # Perform NQA test
                self.perform_nqa_tests(f"{host}:{port}", tn)
                # After NQA test, performance_metrics and summaries should be updated
                performance_metrics = self.performance_metrics.get(f"{host}:{port}")
                performance_summary = self.performance_summaries.get(f"{host}:{port}")

                if performance_metrics and performance_summary:
                    result_data["nqa_result"] = performance_metrics
                    result_data["performance_evaluation"] = self.evaluate_network_performance(performance_metrics)
                    result_data["performance_summary"] = performance_summary
                else:
                    result_data["performance_summary"] = "NQA 测试未完成或失败。"
            else:
                result_data["performance_summary"] = "未找到 OSPF 路由，无法进行 NQA 测试。"
        else:
