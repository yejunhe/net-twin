...
def annotate_network_topology(network_topology: Dict[str, Any],
    ...
    for host_port, device in telnet_devices.items():
        ...
        for interface in interfaces:
            ...
            # 检查接口名称是否已经是完整形式（如 'Ethernet1/0/0' 或 'Ethernet 1/0/0'）
            if re.match(r'^[A-Za-z]+\s?\d+/\d+/\d+$', interface_name):
                # 已经是完整形式，移除空格以标准化接口名称
                standardized_interface_name = interface_name.replace(" ", "")
                required_block_name = f"interface {standardized_interface_name}"
            else:
                # 映射接口类型
