...
class RouterManager:
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        interface_status = {}
        missing_ip_connections = []

        # Create a mapping from sysname to host_port for easy lookup
        sysname_to_host_port = {v: k for k, v in self.telnet_sysnames.items()}

        # Process network connections to add IP status
        if self.network_connections:
            for conn in self.network_connections:
                ...
                node1_ip_configured = False
                node2_ip_configured = False

                # Check node1 interface
                if host_port1:
                    interfaces1 = self.telnet_configurations.get(host_port1, [])
                    iface1_config = next((iface for iface in interfaces1 if iface['Interface'].lower() == interface1.lower()), None)
                    if iface1_config:
                        node1_ip_configured = True

                # Check node2 interface
                if host_port2:
                    interfaces2 = self.telnet_configurations.get(host_port2, [])
                    iface2_config = next((iface for iface in interfaces2 if iface['Interface'].lower() == interface2.lower()), None)
                    if iface2_config:
                        node2_ip_configured = True

                # Determine IP status
                if node1_ip_configured and node2_ip_configured:
                    ip_status = 'both configured'
                elif node1_ip_configured and not node2_ip_configured:
                    ip_status = 'node2 interface not configured'
                elif not node1_ip_configured and node2_ip_configured:
                    ip_status = 'node1 interface not configured'
                else:
                    ip_status = 'both interfaces not configured'

                conn['ip_status'] = ip_status

                # If any interface is not configured, add to missing_ip_connections
                if 'not configured' in ip_status:
                    missing_ip_connections.append(conn)
        ...
        return {
            ...
            "missing_ip_connections": missing_ip_connections  # Added field for connections with missing IP
        }
...
def write_interface_status(data_txt_path: str, mapping: Dict[str, Any]):
    """
    将接口状态和未配置IP的连接状态写入 data.txt，格式如下：
    ...
    未配置IP的连接:
        连接: sysname1:interface1 <-> sysname2:interface2
    """
    try:
        with open(data_txt_path, 'w', encoding='utf-8') as f:
            telnet_devices = mapping.get("telnet_devices", {})
            missing_ip_connections = mapping.get("missing_ip_connections", [])
            ...
            # 写入未配置IP的连接信息
            if missing_ip_connections:
                f.write("未配置IP的连接:\n")
                for conn in missing_ip_connections:
                    node1 = conn['node1']
                    interface1 = conn['interface1']
                    node2 = conn['node2']
                    interface2 = conn['interface2']
                    ip_status = conn.get('ip_status', '未知状态')
                    f.write(f"    连接: {node1}:{interface1} <-> {node2}:{interface2} (状态: {ip_status})\n")
            else:
                f.write("所有连接的接口均已配置IP地址。\n")
