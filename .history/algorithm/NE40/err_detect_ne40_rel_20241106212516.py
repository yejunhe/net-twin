...
class RouterManager:
    def __init__(self, telnet_info: Dict[str, Any], max_workers: int = 20):
        ...
        self.telnet_tracert_output: Dict[str, str] = {}  # Tracert output per device
                ...
                with self.telnet_lock:
                    ...
                    if 'display ip routing-table' in command_outputs:
                        ...
                        if max_cost_dest_ip:
                            ...
                            # 执行 tracert 命令
                            tracert_output = self.execute_tracert(tn, max_cost_dest_ip)
                            if tracert_output:
                                self.telnet_tracert_output[key] = tracert_output
                        else:
                            logging.warning(f"[{tn.host}:{tn.port}] 未找到 Cost 最大的 Destination/Mask。")
                    else:
    ...
    def execute_tracert(self, tn: telnetlib.Telnet, dest_ip: str) -> Optional[str]:
        """
        Execute the 'tracert' command to the specified destination IP and return the output.

        :param tn: Telnet connection object.
        :param dest_ip: Destination IP address to traceroute.
        :return: Output of the 'tracert' command.
        """
        try:
            ...
            time.sleep(5)  # Wait longer for traceroute to complete
            tracert_output = tn.read_very_eager().decode('ascii', errors='ignore')
            logging.debug(f"[{tn.host}:{tn.port}] Output for '{tracert_cmd}':\n{tracert_output}")
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            ...
            tracert_output = self.telnet_tracert_output.get(host_port)
            ...
            if tracert_output:
                logging.debug(f"[{host_port}] Tracert output:\n{tracert_output}")
            ...
            # 新增：处理 tracert 输出
            if tracert_output:
                tracert_outputs[host_port] = tracert_output
                logging.info(f"[{host_port}] Tracert 输出已记录。")
            else:
                tracert_outputs[host_port] = "未执行 Tracert 或 Tracert 失败。"
        ...
        return {
            "telnet_devices": {
                host_port: {
                    ...
                    "tracert_output": tracert_outputs.get(host_port, "")
                }
                for host_port, sysname in self.telnet_sysnames.items()
            },
        ...
        try:
            with open(data_txt_path, 'w', encoding='utf-8') as f:
                ...
                for host_port, device_info in telnet_devices.items():
                    ...
                    tracert_output = device_info.get("tracert_output", "")
                    ...
                    if tracert_output and tracert_output.strip():
                        f.write(f"    Tracert 输出:\n")
                        tracert_lines = tracert_output.strip().splitlines()
                        for line in tracert_lines:
                            f.write(f"        {line}\n")
                    else:
                        f.write(f"    Tracert 输出: 未执行 Tracert 或 Tracert 失败。\n")
