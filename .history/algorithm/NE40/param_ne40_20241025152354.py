...
class RouterManager:
    ...
    def parse_display_bgp_all_summary(self, output: str) -> Optional[Dict[str, Any]]:
        """
        解析 'display bgp all summary' 命令的输出，提取BGP信息。

        :param output: 命令的输出。
        :return: 包含BGP本地Router ID、本地AS号码、总邻居数量、已建立状态的邻居数量、
                以及未建立状态的邻居列表的字典。如果解析失败，返回None。
        """
        bgp_info = {
        ...
        lines = output.splitlines()
        logging.debug("解析 'display bgp all summary' 的输出。")

        # 定义正则表达式模式
        router_id_regex = re.compile(r'BGP\s+local\s+router\s+ID\s*:\s*(\d+\.\d+\.\d+\.\d+)', re.IGNORECASE)
        local_as_regex = re.compile(r'Local\s+AS\s+number\s*:\s*(\d+)', re.IGNORECASE)
        total_peers_regex = re.compile(r'Total\s+number\s+of\s+peers\s*:\s*(\d+)', re.IGNORECASE)
        established_peers_regex = re.compile(r'Peers\s+in\s+established\s+state\s*:\s*(\d+)', re.IGNORECASE)
        ...
        in_peer_table = False  # 标志是否在邻居表中

        for line in lines:
            # 提取本地Router ID
            if bgp_info["local_router_id"] is None:
                match = router_id_regex.search(line)
                if match:
                    bgp_info["local_router_id"] = match.group(1)
                    logging.debug(f"检测到BGP本地Router ID: {bgp_info['local_router_id']}")
                    continue

            # 提取本地AS号码
            if bgp_info["local_as_number"] is None:
                match = local_as_regex.search(line)
                if match:
                    bgp_info["local_as_number"] = match.group(1)
                    logging.debug(f"检测到本地AS号码: {bgp_info['local_as_number']}")
                    continue
            ...
            if bgp_info["total_peers"] == 0:
                match = total_peers_regex.search(line)
                if match:
                    bgp_info["total_peers"] = int(match.group(1))
                    logging.debug(f"检测到总BGP邻居数量: {bgp_info['total_peers']}")
                    continue

            # 提取已建立状态的邻居数量
            if bgp_info["established_peers"] == 0:
                match = established_peers_regex.search(line)
                if match:
                    bgp_info["established_peers"] = int(match.group(1))
                    logging.debug(f"检测到已建立状态的BGP邻居数量: {bgp_info['established_peers']}")
                    continue
            ...
            if in_peer_table:
                ...
                if match:
                    ...
                    if state != 'established':
                        ...
                        logging.debug(f"检测到未建立状态的BGP邻居: IP={peer_ip}, AS={peer_as}, State={state.capitalize()}")
                else:
                    logging.debug(f"无法匹配的BGP邻居行: {line}")

        # 检查是否成功解析
        if bgp_info["local_router_id"] and bgp_info["local_as_number"]:
            logging.info(f"提取的BGP信息: {bgp_info}")
            return bgp_info
        else:
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            ...
            logging.debug(f"[{host_port}] OSPF配置的接口: {ospf_interfaces_lower}")
            logging.debug(f"[{host_port}] ISIS配置的接口: {isis_interfaces_lower}")
            if bgp_info:
                logging.debug(f"[{host_port}] BGP信息: {bgp_info}")
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            ...
            # Determine ISIS status based on 'display isis interface'
            if isis_interfaces:
                isis_status[host_port] = "ISIS 配置正常"
            else:
                isis_status[host_port] = "ISIS 未配置"
    ...
    # Store interface status information
        ...
        # Extract BGP info
            ...
            if bgp_info:
                if bgp_info["total_peers"] > 0:
                    logging.info(f"[{host_port}] BGP 本地 Router ID: {bgp_info['local_router_id']}")
                    logging.info(f"[{host_port}] BGP 本地 AS Number: {bgp_info['local_as_number']}")
                    logging.info(f"[{host_port}] BGP 总邻居数量: {bgp_info['total_peers']}")
                    logging.info(f"[{host_port}] BGP 建立状态的邻居数量: {bgp_info['established_peers']}")
                    if bgp_info["non_established_peers"]:
                        logging.info(f"[{host_port}] BGP 未建立状态的邻居: {bgp_info['non_established_peers']}")
                    else:
                        logging.info(f"[{host_port}] 所有 BGP peers 均处于 Established 状态。")
...
def write_interface_status(data_txt_path: str, mapping: Dict[str, Any]):
    """
    将接口状态和OSPF/ISIS/BGP状态写入 data.txt，格式如下：
