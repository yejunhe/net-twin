...
def write_interface_status(output_path: str, telnet_devices: Dict[str, Any]):
    """
    将接口状态写入到 data.txt 文件中，格式为：
    Node: <sysname>
        <接口名称>接口已配置
        <接口名称>接口未配置
    """
    try:
        with open(output_path, 'w', encoding='utf-8') as f:
            for host_port, device_info in telnet_devices.items():
                ...
                f.write(f"Node: {sysname}\n")
                for status in device_info.get('interface_status', []):
                    f.write(f"    {status}\n")
                f.write("\n")
        logging.info(f"Interface statuses written to {output_path}")
    except IOError as e:
        logging.error(f"Error writing to interface status file: {e}")
        sys.exit(1)
...
def main(input_path: str):
    ...
    # 写入接口状态到 data.txt
    write_interface_status('data.txt', mapping.get('telnet_devices', {}))

    # 写入其他输出内容到 /tmp/param_ne40.json
    write_output('/tmp/param_ne40.json', mapping)
