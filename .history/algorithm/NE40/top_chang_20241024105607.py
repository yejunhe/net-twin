...
class RouterManager:
    def __init__(self, telnet_info: Dict[str, Any], max_workers: int = 20):
        ...
        self.telnet_configurations: Dict[str, Any] = {}  # Changed type to Any to hold parsed configurations
        self.network_connections: Optional[list] = None
        self.routing_tables: Dict[str, Any] = {}  # Store parsed routing table information
        ...
        if output:
            ...
            if sysname:
                ...
                with self.telnet_lock:
                    ...
                    # Parse and store the routing table configuration instead of raw output
                    routing_table = self.parse_routing_table(output)
                    self.telnet_configurations[key] = routing_table
                    self.routing_tables[key] = routing_table
        else:
            logging.warning(f"[{tn.host}:{tn.port}] No output received from Telnet commands.")
        return output

    def parse_routing_table(self, routing_output: str) -> Dict[str, Any]:
        """
        Parse the 'display ip routing-table' command output.
        Extract destination, proto, cost, and nexthop for each interface.
        """
        routing_table = {}
        lines = routing_output.splitlines()
        header_found = False
        headers = ["Destination/Mask", "Proto", "Pre", "Cost", "Flags", "NextHop", "Interface"]

        # Define regex pattern
        pattern = re.compile(
            r'^\s*(?P<destination>\S+)\s+'
            r'(?P<proto>\S+)\s+'
            r'(?P<pre>\d+)\s+'
            r'(?P<cost>\d+)\s+'
            r'(?P<flags>\S+)\s+'
        ...
        for line in lines:
            ...
            if match:
                ...
                if interface not in routing_table:
                ...
                routing_table[interface].append(route_entry)
            else:
                # Handle lines with empty Destination (continuation lines)
                ...
                if cont_match and current_destination:
                    proto = cont_match.group('proto')
                    pre = cont_match.group('pre')
                    cost = cont_match.group('cost')
                    flags = cont_match.group('flags')
                    nexthop = cont_match.group('nexthop')
                    ...
                    logging.warning(f"Unmatched line format: {line}")

        logging.info("Routing table parsed successfully.")
