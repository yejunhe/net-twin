...
class RouterManager:
    def __init__(self, telnet_info: Dict[str, Any], max_workers: int = 20):
        ...
        self.routing_info: Dict[str, list] = {}
        ...
        if output:
            ...
            if sysname:
                ...
                with self.telnet_lock:
                    ...
                    self.parse_routing_table(output, key)
        else:
            logging.warning(f"[{tn.host}:{tn.port}] No output received from Telnet commands.")
        return output

    def parse_routing_table(self, routing_output: str, key: str):
        try:
            routing_entries = []
            for line in routing_output.splitlines():
                match = re.match(r'^(\S+/\d+)\s+(\S+)\s+\d+\s+(\d+)\s+.*?\s+(\S+)\s+(\S+)$', line)
                if match:
                    destination = match.group(1)
                    proto = match.group(2)
                    cost = match.group(3) if proto.lower() != 'direct' else None
                    nexthop = match.group(4) if proto.lower() != 'direct' else None
            ...
            self.routing_info[key] = routing_entries
            logging.info(f"Parsed routing table for {key}")
        except Exception as e:
            logging.error(f"Error parsing routing table for {key}: {e}")
        ...
        return {
            "telnet_devices": {
                host_port: {
                    ...
                    "routing_info": self.routing_info.get(host_port, [])
                }
