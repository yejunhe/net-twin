...
class RouterManager:
    ...
    def execute_telnet_commands(self, tn: telnetlib.Telnet, commands: List[str], quit_cmd: bytes) -> Dict[str, str]:
        ...
        try:
            ...
            if prompt:
                if prompt.endswith('[]'):
                    # 如果提示符以 '[]' 结尾，先发送 'q' 再发送其他命令
                    modified_commands = ['q'] + commands
                    logging.info(f"[{tn.host}:{tn.port}] 提示符为 '[]'，在命令前插入 'q'。")
                else:
                    # 如果提示符为 '<>' 或其他，直接执行命令
                    modified_commands = commands
            else:
                # 如果无法检测到提示符，直接执行命令并记录警告
                modified_commands = commands
                logging.warning(f"[{tn.host}:{tn.port}] 无法检测到提示符。将继续执行默认命令。")
            ...
            for cmd in modified_commands:
                ...
                logging.debug(f"[{tn.host}:{tn.port}] '{cmd}' 的输出:\n{cmd_output}")

            # 执行所有命令后，检查是否需要发送退出命令
            prompt_after_commands = self.get_prompt(tn)
            logging.debug(f"[{tn.host}:{tn.port}] 执行命令后的提示符: {prompt_after_commands}")
