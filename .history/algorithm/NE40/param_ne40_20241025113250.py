...
class RouterManager:
    ...
    # Patterns
        router_id_regex = re.compile(r'BGP local router ID\s*:\s*(\d+\.\d+\.\d+\.\d+)')
        local_as_regex = re.compile(r'Local AS number\s*:\s*(\d+)')
        total_peers_regex = re.compile(r'Total number of peers\s*:\s*(\d+)')
        established_peers_regex = re.compile(r'Peers in established state\s*:\s*(\d+)')
        ...
        # Identify start of peer table
            ...
            # Parse peer entries
                ...
                if len(parts) >= 7:
                    ...
                    up_down = parts[5]
                    state = parts[6]
                # RtRcv and RtAdv may or may not be present
                # Some outputs may have additional columns
