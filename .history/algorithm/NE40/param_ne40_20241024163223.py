...
def write_output(data_txt_path: str, tmp_folder_path: str, data: Dict[str, Any]):
    try:
        # 确保 tmp 文件夹存在
        os.makedirs(tmp_folder_path, exist_ok=True)
        tmp_output_path = os.path.join(tmp_folder_path, 'output.json')

        # 提取接口状态信息
        interface_status_all = []
        telnet_devices = data.get("telnet_devices", {})
        for host_port, device_info in telnet_devices.items():
            status_list = device_info.get("interface_status", [])
            interface_status_all.extend(status_list)

        # 写入 data.txt
        with open(data_txt_path, 'w', encoding='utf-8') as f:
            for status in interface_status_all:
                f.write(status + '\n')
        logging.info(f"Interface status written to {data_txt_path}")

        # 写入 tmp 文件夹下的 output.json
        with open(tmp_output_path, 'w', encoding='utf-8') as f:
            # 移除 interface_status 字段
...
def main(input_path: str, data_txt_path: str, tmp_folder_path: str):
    ...
    # Write outputs
    write_output(data_txt_path, tmp_folder_path, mapping)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--data", required=True, help="Path to output data.txt file.")
    parser.add_argument("-t", "--tmp", required=True, help="Path to tmp folder for other data.")
    args = parser.parse_args()
    main(args.input, args.data, args.tmp)
