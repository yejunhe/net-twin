...
def generate_summary(network_topology: Dict[str, Any],
    ...
    for device in telnet_devices.values():
        sysname = device.get("sysname")
        if sysname:
            sysname_to_device[sysname] = device

    for node in network_topology.get("nodes", []):
        node_name = node.get("name")
        device = sysname_to_device.get(node_name)
        if not device:
            # 节点没有对应的telnet_device信息
            protocols = []
            interfaces = [{"名称": iface.get("name"), "配置状态": "已配置" if iface.get("configured", False) else "未配置"}
            ...
            for protocol in all_protocols:
                ...
                if block_key in device.get("blocks", {}):
                    protocol_statuses[protocol] = device["blocks"][block_key]["status"]
                    if device["blocks"][block_key]["status"] == "已配置":
                        protocols.append(protocol)
                else:
                    protocol_statuses[protocol] = "未配置"

            interfaces = [{"名称": iface.get("name"), "配置状态": "已配置" if iface.get("configured", False) else "未配置"}
                          for iface in node.get("interfaces", [])]
        node_summary = {
            "名称": node_name,
            "协议": protocol_statuses if protocol_statuses else "无",
            "接口": interfaces
        }
