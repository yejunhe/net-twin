...
class RouterManager:
    def __init__(self, telnet_info: Dict[str, Any], max_workers: int = 20):
        ...
        self.telnet_configurations: Dict[str, Dict[str, Any]] = {}  # 结构化接口信息及OSPF状态
    ...
    def execute_telnet_commands(self, tn: telnetlib.Telnet, commands: List[str], quit_cmd: bytes) -> Dict[str, str]:
        """
        执行一系列Telnet命令，并将每个命令的输出存储在字典中。

        :param tn: 已连接的Telnet会话。
    ...
    def get_configuration_via_telnet(self, tn: telnetlib.Telnet, image_type: str) -> Optional[Dict[str, Any]]:
        ...
        if command_outputs:
            ...
            if sysname:
                ...
                with self.telnet_lock:
                    ...
                    # 调用解析方法
                    ip_interfaces = self.parse_display_ip_interface_brief(command_outputs.get('display ip interface brief', ''))
                    ospf_interfaces = self.parse_display_ospf_interface(command_outputs.get('display ospf interface', ''))
                    self.telnet_configurations[key] = {
                        "interfaces": ip_interfaces,
    ...
    def collect_results(self) -> Dict[str, Any]:
        """
        收集所有结果，并进行接口匹配，输出接口配置状态。
        """
        # 存储接口状态信息
        interface_status = {}

        for host_port, device_info in self.telnet_configurations.items():
            sysname = self.telnet_sysnames.get(host_port, "未知节点")
            node_interfaces = self.node_interfaces.get(sysname, [])
            telnet_interfaces = device_info.get("interfaces", [])
            ...
            ospf_interface_names = [iface['Interface'] for iface in ospf_interfaces if isinstance(iface, dict)]

            logging.debug(f"[{host_port}] Telnet 获取的接口列表: {telnet_interface_names}")
            logging.debug(f"[{host_port}] 配置了OSPF的接口列表: {ospf_interface_names}")
            ...
            for iface in node_interfaces:
                ...
                # 判断接口是否配置了OSPF
                if iface_formatted in ospf_interface_names:
                    ospf_status = "已配置OSPF"
                else:
                    ospf_status = "未配置OSPF"

                status = f"{iface_formatted}接口 - {config_status}, {ospf_status}"
        ...
        return {
            "telnet_devices": {
                host_port: {
                    "sysname": sysname,
                    "interfaces": self.telnet_configurations.get(host_port, {}).get("interfaces", []),
                    "ospf_interfaces": self.telnet_configurations.get(host_port, {}).get("ospf_interfaces", []),
    ...
    def parse_display_ospf_interface(self, output: str) -> List[Dict[str, str]]:
        """
        解析 'display ospf interface' 命令的输出，提取配置了OSPF的接口信息。

        :param output: 命令的输出内容。
        :return: 配置了OSPF的接口信息列表，每个接口信息为字典。
        """
        lines = output.splitlines()
        ospf_interfaces = []
        header_found = False
        ...
        # 正则表达式匹配Area行
        area_regex = re.compile(r'^Area:\s+([\d\.]+)\s+\(.*\)$')

        # 正则表达式匹配接口行
        interface_regex = re.compile(
            r'^(?P<interface>\S+)\s+'
            r'(?P<ip_address>(?:\d{1,3}\.){3}\d{1,3}|\S+)\s+'
            r'(?P<type>\S+)\s+'
            r'(?P<state>\S+)\s+'
            r'(?P<cost>\d+)\s+'
            r'(?P<pri>\d+)'
        ...
        for line in lines:
            ...
            # 检查Area行
            area_match = area_regex.match(line)
            if area_match:
                current_area = area_match.group(1)
                logging.debug(f"Detected Area: {current_area}")
                continue

            # 寻找表头
            if not header_found:
                if re.match(r'^Interface\s+IP Address\s+Type\s+State\s+Cost\s+Pri', line):
                    header_found = True
                    continue
                else:
                    continue
        ...
        logging.debug(f"Parsed OSPF interfaces: {ospf_interfaces}")
...
def write_interface_status(data_txt_path: str, mapping: Dict[str, Any]):
    ...
    节点: sysname1 (host:port)
        接口: Interface1接口 - 已配置, 已配置OSPF
        接口: Interface2接口 - 未配置, 未配置OSPF
    节点: sysname2 (host:port)
        接口: Interface3接口 - 已配置, 已配置OSPF
        接口: Interface4接口 - 未配置, 未配置OSPF
    """
    try:
        with open(data_txt_path, 'w', encoding='utf-8') as f:
            ...
            for host_port, device_info in telnet_devices.items():
                ...
                for status in interface_status_list:
                    # 假设 status 格式为 "InterfaceName接口 - 已配置, 已配置OSPF"
                    f.write(f"    接口: {status}\n")
                f.write("\n")  # 添加空行以分隔不同节点
