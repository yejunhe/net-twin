...
class RouterManager:
    ...
    def check_required_blocks(self, cleaned_config: str) -> Dict[str, Dict[str, Any]]:
        ...
        for block, patterns in self.required_blocks.items():
            if patterns is None:
                ...
                if pattern.search(cleaned_config):
                    # 提取关键字所在的行及其后续内容直到遇到 '#'
                    extracted_content = self.extract_content_until_hash(block, cleaned_config)
                    checks[block] = {
                        "status": "已配置",
    ...
    def extract_content_until_hash(self, keyword: str, cleaned_config: str) -> str:
        """
        提取包含关键字的行及其后续的所有行，直到遇到 '#' 或配置结束。
        """
        lines = cleaned_config.splitlines()
        extracted_segments = []
        capture = False
        current_segment = []
        ...
        for line in lines:
            ...
            elif re.search(r'\b' + re.escape(keyword) + r'\b', line, re.IGNORECASE):
                capture = True
                current_segment.append(line)
        ...
        return '; '.join(extracted_segments) if extracted_segments else ""
