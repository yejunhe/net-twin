...
class RouterManager:
    def __init__(self, telnet_info: Dict[str, Any], max_workers: int = 20):
        ...
        self.routing_tables: Dict[str, Any] = {}  # 新增用于存储路由表信息
    ...
    def parse_routing_table(self, routing_output: str) -> Dict[str, Any]:
        """
        解析 'display ip routing-table' 的输出内容。
        将每个接口下对应的destination，proto，cost，nexthop整理出来。
        proto为direct的不用记录cost和nexthop。
        """
        routing_table = {}
        ...
        for line in lines:
            ...
            if match:
                ...
                # 处理那些Destination为空的行（续行）
                # 例如:
                #   10.0.12.0/24  OSPF    10   2             D   10.0.13.1                                Ethernet1/0/2
        ...
        return {
            "telnet_devices": {
                host_port: {
                    ...
                    "routing_table": self.routing_tables.get(host_port, {})
                }
