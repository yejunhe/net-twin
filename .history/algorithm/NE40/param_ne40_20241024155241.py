...
logging.basicConfig(
    level=logging.INFO,  # 可以根据需要调整日志级别，例如 DEBUG
...
class RouterManager:
    def __init__(self, telnet_info: Dict[str, Any], max_workers: int = 20):
        ...
        self.telnet_configurations: Dict[str, Any] = {}  # 支持结构化数据
    ...
    def collect_results(self) -> Dict[str, Any]:
        return {
            "telnet_devices": {
                host_port: {
                    "sysname": sysname,
                    "interfaces": self.telnet_configurations.get(host_port, 'No config')
                }
    ...
    def parse_display_ip_interface_brief(self, output: str) -> list:
        """
            ...
            if not header_found:
                ...
                if match:
                    ...
                    if ip_address.lower() != 'unassigned':
                        ...
                        # 将接口信息格式化为对齐的字符串，保留表头格式
                        formatted_line = f"{interface_info['Interface']:<35} {interface_info['IP Address/Mask']:<20} {interface_info['Physical']:<10} {interface_info['Protocol']:<10} {interface_info['VPN']}"
                        interfaces.append(formatted_line)
                else:
                    # 如果行不匹配预期格式，保留原始行（可选）
                    interfaces.append(line)
