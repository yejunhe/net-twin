    ...
    def clean_configuration(self, raw_config: str) -> str:
        ...
        for block in blocks:
            ...
            if 'interface ' in block:
                logging.debug("Removing interface configuration block.")
                continue
            # 跳过包含其他无用内容的块（根据需要添加更多条件）
            if any(unwanted in block for unwanted in [
                'license',
                'warranty',
                'local-aaa-server',
                'pki domain default'
            ]):
                logging.debug("Removing unwanted configuration block.")
                continue
            # 逐行处理块内容，删除特定行
            ...
            for line in lines:
                ...
                # 删除包含 'undo shutdown' 的行
                if 'undo shutdown' in stripped_line:
                    continue
                # 删除其他无用的具体命令（根据需要添加）
                if any(cmd in stripped_line for cmd in [
                    'undo FTP server-source',
                    'undo FTP ipv6 server-source',
                    'undo telnet server enable',
                    'undo telnet ipv6 server enable',
                    'undo snmp-agent protocol source-status',
                    'undo snmp-agent proxy protocol source-status',
                    'ssh server-source',
        ...
        if output:
            ...
            if sysname:
                ...
                with self.telnet_lock:
                    ...
                    self.telnet_configurations[key] = cleaned_output
        else:
