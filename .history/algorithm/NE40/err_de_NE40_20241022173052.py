...
class RouterManager:
    def __init__(self, telnet_info: Dict[str, Any], max_workers: int = 20):
        ...
        # Define required configuration blocks to check with their check patterns
        # The check patterns can be substrings or regex patterns
        # If the value is None, only the existence of the block is checked
        self.required_blocks = {
            'sysname': None,  # No specific pattern, existence suffices
            'bgp': None,
            'ospf': None,
            'isis': None,
            ...
            'interface NULL0': None  # No specific pattern
            # 添加更多需要检查的配置块及其检查模式
        }
