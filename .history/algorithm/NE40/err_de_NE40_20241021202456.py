...
class RouterManager:
    ...
    def clean_configuration_output(self, output: str) -> str:
        # Remove unnecessary lines and repeated sections from the configuration output
        cleaned_lines = []
        lines = output.splitlines()
        for line in lines:
            if not line.startswith('<') and not line.startswith('Info:') and not line.startswith('scr 0 t'):
