...
class RouterManager:
    ...
    def connect_via_docker(self, docker_id):
        ...
        try:
            ...
            # 获取初始行信息，例如 'root@FR2:/#'
            initial_output = self.execute_docker_commands(container_id, ['echo root@$(hostname):/#'])
            print(f"Initial Docker command output:\n{initial_output}")

            # 提取 sysname
            sysname = self.extract_sysname_from_initial_output(initial_output)
            if sysname:
                print(f"Detected sysname from Docker container {docker_id}: {sysname}")
                self.docker_sysnames[docker_id] = sysname
            else:
                print(f"Sysname not detected in Docker container {docker_id} initial output.")
            ...
            config_output = self.execute_docker_commands(container_id, commands)
            print(f"Docker command output:\n{config_output}")

            self.docker_configurations[docker_id] = config_output
    ...
    def extract_sysname_from_initial_output(self, output):
        """
        从 Docker 容器的初始输出中提取 sysname。
        假设初始输出格式为 'root@FR2:/#'，其中 'FR2' 是 sysname。
        """
        try:
            sysname = None
            lines = output.splitlines()
            for line in lines:
                ...
                if '@' in line and ':' in line:
                    ...
                    if len(parts) > 1:
                        host_part = parts[1]
                        sysname = host_part.split(':')[0]
                        print(f"Extracted sysname: {sysname}")  # 调试信息
                        break
            print(f"Final extracted sysname: {sysname}")  # 调试信息
            return sysname
        except Exception as e:
            print(f"Error extracting sysname from initial Docker output: {e}")
            return None
