...
class RouterManager:
    ...
    def connect_via_docker(self, docker_id):
        ...
        try:
            ...
            # 组合成一个单一的vtysh命令，使用 -c 选项依次执行命令
            commands = [
                'vtysh -c "terminal length 0" -c "show ip ospf route"'
            ]
            ...
            post_output = self.execute_docker_commands(container_id, post_commands)
            output += post_output
            print(f"Docker post-command output:\n{post_output}")
    ...
    def extract_sysname_from_docker_output(self, output):
        ...
        try:
            ...
            for line in lines:
                ...
                # 检查 'FR2#' 格式
                if line.endswith('#'):
                    sysname_candidate = line.rstrip('#').strip()
