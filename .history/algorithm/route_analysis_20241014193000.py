...
class RouterManager:
    ...
    def execute_telnet_commands_generic(self, tn):
        """
        Execute a predefined list of commands on a Telnet connection for non-H3C devices.
        :param tn: telnetlib.Telnet object
        :return: Output from the Telnet session
        """
        try:
    ...
    def get_configuration_via_telnet(self, tn, image_type):
        ...
        try:
            if "h3c" in image_type:
                ...
                # 对于其他设备，使用通用的方法执行命令
                config_output = self.execute_telnet_commands_generic(tn)
    ...
    def connect_via_docker(self, docker_id):
        ...
        try:
            ...
            # 发送后续命令（根据需要添加）
            # 这里可以根据具体需求添加更多的 Docker 命令

            # 示例：执行另一个命令
            additional_commands = [
                'vtysh -c "show running-config"'
            ]
            additional_output = self.execute_docker_commands(container_id, additional_commands)
            print(f"Additional Docker command output:\n{additional_output}")
            self.configurations[docker_id] = output + additional_output
    ...
    def extract_sysname_from_docker_output(self, output):
        ...
        假设 sysname 以类似于 H3C 格式的方式显示，例如 <H1>。
        """
        try:
            sysname = None
            lines = output.splitlines()
...
class TopologyMapper:
    ...
    def map_topology(self):
        ...
        for docker_id, sysname in self.router_manager.sysnames.items():
            if sysname == "Connected via Docker" or sysname == "Unknown":
                mapping[docker_id] = {
                    'docker_id': docker_id,
                    'sysname': sysname,
