...
class RouterManager:
    def __init__(self, telnet_info: Dict[str, Any], max_workers: int = 20):
        ...
        self.required_blocks = {
            ...
            # 接口配置块及其需要检查的关键字
            'interface GigabitEthernet1/0': ['ip address', 'port link-mode route'],
            'interface GigabitEthernet2/0': ['ip address', 'port link-mode route'],
            'interface GigabitEthernet3/0': ['ip address', 'port link-mode route'],
            'interface LoopBack0': ['ip address', 'port link-mode route'],
