...
class RouterManager:
    ...
    def parse_display_ip_interface_brief(self, output: str) -> List[Dict[str, str]]:
        """
        Parse the output of 'display ip interface brief', include all interfaces, and return structured data.
    ...
    def standardize_interface_name(self, interface: str) -> str:
        """
        Standardize the interface name format if needed.
        For example, convert 'GE1/0' to 'GigabitEthernet1/0'
        Adjust this method based on your naming conventions.

        :param interface: Original interface name.
        :return: Standardized interface name.
        """
        # Example standardization, adjust according to actual requirements
        if interface.startswith('GE'):
            return interface.replace('GE', 'GigabitEthernet')
        elif interface.startswith('Loop'):
            return interface  # No change for loopback interfaces
        else:
            return interface  # Add more conditions as needed
