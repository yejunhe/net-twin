...
class CombinedScriptManager:
    ...
    def run(self):
        ...
        if topology_differences is None:
            # No history exists, save current as history
            try:
                history_manager = HistoryManager(lab_id)
                unl_parser = UnlParser(lab_id)
                current_json = unl_parser.parse_unl_to_json()
                history_manager.save_history(current_json)
                topology_report = "首次运行，无历史记录进行比较。当前拓扑已保存为历史记录。"
                logging.info("History has been saved for the first run.")
            except Exception as e:
                topology_report = f"首次运行时尝试保存历史记录失败。错误信息: {e}"
                logging.error(f"首次运行时保存历史记录失败: {e}")
