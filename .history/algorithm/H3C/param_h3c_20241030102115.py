...
class RouterManager:
    def __init__(self, telnet_info: Dict[str, Any], max_workers: int = 20):
        ...
        self.telnet_isis_system_ids: Dict[str, str] = {}  # 新增：ISIS System ID per device
            ...
            if sysname:
                ...
                with self.telnet_lock:
                    ...
                    if 'display isis peer' in command_outputs:
                        ...
                        if isis_peers_info:
                            self.telnet_isis_system_ids[key] = isis_peers_info.get('local_system_id', "未知 System ID")
    ...
    def parse_display_isis_interface(self, output: str) -> List[str]:
        ...
        for line in lines:
            ...
            if match:
                ...
                skip_header = True  # 下一个数据行是表头，需跳过
            ...
            if in_interface_block and current_interface:
                ...
                if len(state_parts) >= 3:
                    ...
                    logging.debug(f"未匹配的接口状态行: {line}")
    ...
    def parse_display_isis_peer(self, output: str) -> Dict[str, Any]:
        ...
        # 正则表达式匹配本地 System ID，允许前导零
        system_id_regex = re.compile(r'^System ID:\s+(\d{4}\.\d{4}\.\d{4})', re.IGNORECASE)
        # 正则表达式匹配邻居信息行
        peer_entry_regex = re.compile(
            r'^Interface:\s+(?P<interface>\S+)\s+Circuit Id:\s+(?P<circuit_id>\S+)\s+State:\s+(?P<state>\S+)\s+HoldTime:\s+\d+s\s+Type:\s+(?P<type>\S+)\s+PRI:\s+(?P<pri>\d+)',
            re.IGNORECASE
        )
        ...
        if local_system_id:
            # 存储到新的 ISIS System ID 字典中
            # 假设 host_port 可以在调用此方法时传递，或者通过其他方式关联
            # 这里需要确保有 host_port 可用，假设方法签名中有 host_port 参数
            # 如果没有，需要修改方法签名以接受 host_port
            # 这里假设有 host_port 变量
            # self.telnet_isis_system_ids[host_port] = local_system_id
            # 由于当前方法签名没有 host_port，可能需要调整调用方式
            # 这里仅返回结果，存储逻辑在调用者中完成
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            ...
            isis_system_id = self.telnet_isis_system_ids.get(host_port)
            ospf_router_id = self.telnet_router_ids.get(host_port)
            ...
            logging.debug(f"[{host_port}] Telnet fetched standardized interfaces: {telnet_interface_names}")
            ...
            for iface in node_interfaces:
                # Get raw interface name from UNL file
            ...
            # **关键修改开始**
            # 调整ISIS配置判定逻辑：只需检测到本地System ID，无需检测邻居数量
            if isis_system_id:
                isis_status[host_port] = "ISIS 已配置"
            else:
...
def write_interface_status(data_txt_path: str, mapping: Dict[str, Any]):
    ...
    Node: sysname1 (host:port)
        Interface: GigabitEthernet1/0接口配置状态: 已配置IP地址, OSPF已配置, ISIS已配置
        OSPF Status: OSPF 配置正常，邻居 Router IDs: 2.2.2.2, 1.1.1.1
        ISIS Status: ISIS 已配置
...
def main(input_path: str, output_path: str):
    ...
    # 定义 data.txt 的路径，放在 output_path 同一目录下
