...
class RouterManager:
    def __init__(self, telnet_info: Dict[str, Any], max_workers: int = 20):
        ...
        self.required_blocks = {
            ...
            # 接口配置块，仅检查存在性
            # 这些接口名称应与网络拓扑中的接口名称一致
            'interface GigabitEthernet1/0': None,
            'interface GigabitEthernet2/0': None,
            'interface GigabitEthernet3/0': None,
    ...
    def clean_configuration(self, raw_config: str) -> str:
        ...
        对接口配置块进行进一步处理，仅保留那些包含 'port link-mode route' 的接口。
        """
        cleaned_blocks = []
        ...
        for block in blocks:
            ...
            if first_line.startswith('sysname'):
                ...
                logging.debug(f"保留配置块: {lines[0].strip()}")
                continue
            elif first_line.startswith('interface'):
                # 处理 interface 块
                # 检查是否包含 'port link-mode route'
                has_port_link_mode_route = any('port link-mode route' in line.lower() for line in lines)
                if has_port_link_mode_route:
                    cleaned_blocks.append(block)
                    logging.debug(f"保留已配置的接口块: {lines[0].strip()}")
                else:
            ...
            # 可以根据需要添加更多有用的配置块判断条件
                ...
                # 其他不需要的配置块删除
                logging.debug(f"移除不需要的配置块: {lines[0].strip()}")
                continue
    ...
    def check_required_blocks(self, cleaned_config: str) -> Dict[str, Dict[str, Any]]:
        ...
        对于每个配置块，如果配置模式为 None，则仅检查配置块是否存在。
        ...
        for block, patterns in self.required_blocks.items():
            if patterns is None:
                # 对于模式为 None 的配置块，仅检查配置块是否存在
                # 比如接口块
                matching_blocks = [blk.strip() for blk in cleaned_blocks if blk.strip().lower().startswith(block.lower())]
                if matching_blocks:
                    # 提取匹配的块内容，并用分号分隔
...
def annotate_network_topology(network_topology: Dict[str, Any],
    ...
    for host_port, device in telnet_devices.items():
        ...
        for interface in interfaces:
            ...
            # 检查该接口是否已配置
            is_configured = required_block_name in device.get("blocks", {})
            ...
            logging.debug(f"接口 '{interface_name}' 状态: {'已配置' if is_configured else '未配置'}")
