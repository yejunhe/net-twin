...
class RouterManager:
    ...
    def parse_display_isis_peer(self, output: str) -> Tuple[Optional[str], Optional[List[str]]]:
        """
        解析 'display isis peer' 的输出，以提取本地 System ID 和邻居 System IDs。

        :param output: 命令输出。
        :return: 元组 (本地 System ID, 邻居 System IDs 列表)。如果无法解析，返回 (None, None)。
        """
        system_id = None
        neighbors = []
        lines = output.splitlines()
        ...
        # 正则表达式匹配表格行（假设 System ID 是5位数字）
        neighbor_regex = re.compile(
            r'^(?P<neighbor_id>\d{5})\s+'      # Neighbor System ID (假设为5位数字)
            r'(?P<state>\w+)\s+'               # State
            r'(?P<dead_state>\w+)\s+'          # Dead-Time State
            r'(?P<index>\d+)\s+'               # Index
            r'(?P<mtu>\d+)\s+'                 # MTU
            r'(?P<type>\S+)\s+'                # Type
            r'(?P<dis>\S+)'                     # DIS
        ...
        for line in lines:
            ...
            if not system_id:
                ...
                if match:
                    ...
                    logging.debug(f"检测到本地 System ID: {system_id}")
                continue

            # 提取邻居 System IDs
            match = neighbor_regex.match(line)
            if match:
                neighbor_id = match.group('neighbor_id')
                neighbors.append(neighbor_id)
                logging.debug(f"检测到邻居 System ID: {neighbor_id}")

        if system_id:
            logging.info(f"提取到本地 System ID: {system_id} 和邻居 System IDs: {neighbors}")
            return system_id, neighbors
        else:
            logging.warning("未能提取到本地 System ID。")
            return None, None
