...
class RouterManager:
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            ...
            # **关键修改开始**
            # 调整ISIS配置判定逻辑：只需检测到ISIS接口和本地System ID，无需检测邻居数量
            if isis_interfaces and router_id:
                isis_status[host_port] = "ISIS 已配置"
            elif isis_interfaces and not router_id:
                isis_status[host_port] = "ISIS 配置问题：接口配置了 ISIS 但未检测到 System ID"
            else:
