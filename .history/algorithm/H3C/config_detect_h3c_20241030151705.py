...
class RouterManager:
    def __init__(self, telnet_info: Dict[str, Any], max_workers: int = 20):
        ...
        # 为不同设备类型定义命令序列
        self.commands_map = {
            "h3c": (['screen-length disable', 'display current-configuration'], b'quit\n'),
            # 可以根据需要为其他设备类型添加命令序列
        }
        # 定义需要检查的配置块及其对应的检查模式
...
def annotate_network_topology(network_topology: Dict[str, Any],
    ...
    for host_port, device in telnet_devices.items():
        ...
        # 添加调试日志
        logging.debug(f"[{host_port}] blocks: {device.get('blocks', {})}")
        ...
        for interface in interfaces:
            ...
            # 去除键名中的空格
            blocks_cleaned = {k.strip(): v for k, v in device.get("blocks", {}).items()}
