...
class RouterTelnetManager:
    ...
    def connect_and_get_sysnames_routes_and_nqa(self):
        """Connect to each router via Telnet and retrieve sysname, OSPF routes, and NQA results"""
        results = []

        for connection in self.telnet_info["connections"]:
            host = connection["ip"]
            port = connection["port"]
            print(f"连接到 {host}:{port}...")

            sysname, ospf_route, nqa_result = self.get_sysname_and_routing_table(host, port)
...
def main(input_path, output_path):
    ...
    with open(output_path, 'w') as f:
        f.write(json.dumps(results, indent=4, ensure_ascii=False))  # 使用 ensure_ascii=False 保证中文字符的正确显示
    print(f"结果和分析已写入 {output_path}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="获取路由器信息并输出至指定文件")
