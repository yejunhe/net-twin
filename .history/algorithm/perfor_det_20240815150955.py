    ...
    # 匹配容器ID
        ...
        if matched_container_ids:
            for container_id in matched_container_ids:
                ...
                self.output_data["containers"].append({
                    "container_id": container_info["container_id"],
                    "hostname": container_info["hostname"],
                    "frr_conf": container_info["frr_conf"],
...
def main():
    ...
    parser.add_argument("-i", "--input", help="Directory containing the reasoning folder", default=default_reasoning_directory)
    parser.add_argument("-o", "--output", help="JSON output file", default=default_json_output_file)
    
    args = parser.parse_args()

    # 根据是否提供参数来选择路径
    reasoning_directory = args.input if args.input else default_reasoning_directory
    ...
    if not lab_id:
        ...
        if not os.path.exists(unl_file_path):
            ...
            if reader.parse_file():
                ...
                reader.write_output(json_output_file, reasoning_directory)
