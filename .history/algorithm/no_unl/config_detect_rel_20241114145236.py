...
class RouterManager:
    ...
    def parse_display_ospf_peer(self, output: str) -> Tuple[Optional[str], Optional[List[Dict[str, Any]]]]:
        ...
        neighbor_interface_regex = re.compile(r'Area \d+\.\d+\.\d+\.\d+ interface (\S+)\s*\(\S+\)\'s neighbors')
        ...
        for i, line in enumerate(lines):
            ...
            # If current_interface is set, look for neighbor details
            if current_interface:
                ...
                if router_id_match:
                    neighbor_router_id = router_id_match.group(1)
                    logging.debug(f"Detected Neighbor Router ID: {neighbor_router_id}")

                # Address line
                address_match = address_regex.search(line)
                if address_match:
                    neighbor_address = address_match.group(1)
                    logging.debug(f"Detected Neighbor Address: {neighbor_address}")

                # State line
                state_match = state_regex.search(line)
                if state_match:
                    neighbor_state = state_match.group(1)
                    logging.debug(f"Detected Neighbor State: {neighbor_state}")

                # Check if all necessary info has been collected
                if neighbor_router_id and neighbor_address and neighbor_state:
                    # Determine if state is problematic
                    is_problematic = neighbor_state.lower() != 'full'
                    ...
                    # Reset current_interface to avoid duplicate entries
                    current_interface = None
                else:
                    # If not all information is present, log a warning and decide whether to reset or keep current_interface
                    logging.warning(f"Incomplete neighbor information detected on interface {current_interface}.")
                    # Depending on your requirements, you might reset or keep the current_interface
                    # For this example, we'll reset it to avoid infinite loops
                    current_interface = None
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            ...
            if bgp_info:
                ...
                if is_boundary_router and isinstance(bgp_local_as_number, int):
                    as_number = bgp_local_as_number  # Router's local AS number represents its AS
            ...
            # Process OSPF Neighbors Details
        ...
        # Collect protocol configuration issues
        for host_port in ospf_neighbor_issues:
            protocol_issues.add(host_port)

        # Collect BGP configuration issues
        for host_port, bgp_info in bgp_info_dict.items():
            if bgp_info.get("bgp_total_peers", 0) > 0:
                if bgp_info.get("bgp_established_peers", 0) < bgp_info.get("bgp_total_peers", 0):
                    # 存在未建立的 BGP peer
                    bgp_issues.add(host_port)
            elif bgp_info.get("bgp_total_peers", 0) == 0 and bgp_info.get("bgp_local_router_id") != "未知":
                # BGP 已配置但无 peers
                bgp_issues.add(host_port)

        mapping["protocol_issues"]["BGP_issues"] = list(bgp_issues)
        ...
        if ospf_issues or isis_issues or bgp_issues or ospf_neighbor_issues:
            ...
            protocol_issues_evaluation.append("所有节点的 OSPF、BGP 和 ISIS 配置均正常。")
            logging.info("所有节点的 OSPF、BGP 和 ISIS 配置均正常。")
    ...
    def write_interface_status(self, data_txt_path: str, mapping: Dict[str, Any]):
        ...
        :param data_txt_path: 输出的 data.txt 文件路径。
        :param mapping: 结果字典。
        """
        try:
