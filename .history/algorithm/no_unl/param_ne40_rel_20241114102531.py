...
class RouterManager:
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        if ospf_issues or isis_issues or bgp_issues:
        ...
        mapping["evaluation_per_as"]["protocol_issues_evaluation"] = protocol_issues_evaluation
    ...
    def write_interface_status(self, data_txt_path: str, mapping: Dict[str, Any]):
        ...
        try:
            with open(data_txt_path, 'w', encoding='utf-8') as f:
                ...
                protocol_issues = evaluation_per_as.get("protocol_issues_evaluation", [])
                ...
                for host_port, device_info in telnet_devices.items():
                    ...
                    # 输出配置了 OSPF 的接口
                    if ospf_interfaces:
                        ospf_ifaces_str = ', '.join(ospf_interfaces)
                        f.write(f"    配置了 OSPF 的接口: {ospf_ifaces_str}\n")
                    else:
                        f.write(f"    配置了 OSPF 的接口: 无\n")

                    # 输出配置了 ISIS 的接口
                    if isis_interfaces:
                        isis_ifaces_str = ', '.join(isis_interfaces)
                        f.write(f"    配置了 ISIS 的接口: {isis_ifaces_str}\n")
                    else:
                        f.write(f"    配置了 ISIS 的接口: 无\n")
