...
class RouterTelnetManager:
    ...
    def parse_routing_table(self, routing_table):
        ...
        # 定义需要查找的协议列表
        protocols = ['OSPF','ISIS-L1', 'ISIS-L2', 'EBGP']
    ...
    try:
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            for result in results:
                f.write(f"设备: {result['host']}:{result['port']}\n")
                f.write(f"Sysname: {result['sysname'] if result['sysname'] else '未知'}\n")
                if result['ospf_ip']:
                    f.write(f"OSPF 路由 IP: {result['ospf_ip']}\n")
                else:
                    f.write("OSPF 路由 IP: 未找到\n")
                
                if result['nqa_result']:
                    f.write("NQA 测试结果:\n")
                    metrics = result['nqa_result']
                    f.write(f"  延迟 (平均 RTT): {metrics.get('latency', '未知')} ms\n")
                    f.write(f"  抖动 (平均 Jitter): {metrics.get('jitter', '未知')} ms\n")
                    f.write(f"  丢包率: {metrics.get('packet_loss', '未知')} %\n")
                else:
                    f.write("NQA 测试结果: 无\n")
                
                if result['performance_summary']:
                    f.write(f"性能评估总结: {result['performance_summary']}\n")
                else:
                    f.write("性能评估总结: 无法进行网络性能评估。\n")
                
                f.write("-"*50 + "\n\n")
        
        print(f"报告已生成并写入 {output_path}")
    except Exception as e:
        print(f"无法写入输出报告文件 '{output_path}': {e}")
...
def main(input_path, output_path, max_threads=10):
    ...
    try:
        ...
        print(f"无法生成报告文件 '{output_path}': {e}")
