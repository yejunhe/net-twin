...
class RouterManager:
        ...
        if command_outputs:
            ...
            if sysname:
                ...
                with self.telnet_lock:
                    ...
                    # Parse 'display ip routing-table' to prepare for tracert
                    if 'display ip routing-table' in command_outputs:
                        # Already handled in collect_results, no need to parse here
                        pass
                    else:
    ...
    def collect_results(self) -> Dict[str, Any]:
        ...
        including OSPF, ISIS, and BGP statuses. Additionally, execute tracert commands based on routing table.
        """
        # 存储接口状态信息
        interface_status = {}
        ospf_status = {}
        ...
        for host_port, sysname in self.telnet_sysnames.items():
            try:
                ...
                logging.error(f"无法连接到 {host_port}：{e}")
                continue
            ...
            try:
                tn.write(b'q\n')
                tn.close()
                logging.info(f"[{host_port}] Telnet 连接已关闭。")
            except Exception as e:
                logging.error(f"[{host_port}] 关闭 Telnet 连接时出错：{e}")
