        ...
        for i, line in enumerate(lines):
            ...
            if current_interface:
                ...
                if neighbor_router_id and neighbor_address and neighbor_state:
                    ...
                    # If not all information is present, log a warning and reset current_interface
            ...
            for iface_info in telnet_interfaces:
                ...
                if ip_address and ip_address.lower() != 'unassigned':
                    ...
                    if physical_up and protocol_up:
                        ...
                        # If interface status is abnormal, record protocol configuration issues if necessary
                    ...
                    if iface.lower() in ospf_interfaces_lower:
                        ...
                        status += " | 未配置 OSPF"
                        # If OSPF is required but not configured, you might want to add to ospf_issues
                        ospf_issues.add(host_port)
                    ...
                    if iface.lower() in isis_interfaces_lower:
                        ...
                        status += " | 未配置 ISIS"
                        # Similarly, handle ISIS issues if necessary
                        isis_issues.add(host_port)

                    interface_status[host_port].append(status)
                    logging.info(f"[{host_port}] {status}")
                else:
                    # Interfaces without IP addresses are not included in the output
                    continue
