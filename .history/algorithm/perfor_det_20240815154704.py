...
class NodeReader:
    ...
    # 确保输出目录存在
        output_directory = os.path.dirname(txt_output_file)
...
def main():
    ...
    default_txt_output_file = 'data.txt'  # 将输出文件设置为 'data.txt'
    ...
    parser.add_argument("-o", "--output", help="TXT output file", default=default_txt_output_file)
    ...
    if not lab_id:
        ...
        if not os.path.exists(unl_file_path):
            ...
            if reader.parse_file():
                # 调用 write_output 时传递 reasoning_directory 或者直接传递文件路径
