...
class RouterManager:
    ...
    def parse_display_bgp_all_summary(self, output: str) -> Optional[Dict[str, Any]]:
        """
        解析 'display bgp all summary' 输出，提取 BGP 信息并判断路由器是否为边界路由器。

        :param output: 命令输出。
        :return: 包含 BGP 本地路由器 ID、本地 AS 号、总邻居数、已建立邻居数、
                未建立邻居列表以及边界路由器状态的字典。如果解析失败，返回 None。
        """
        bgp_info = {
            ...
            "is_boundary_router": False  # 新增标记，判断是否为边界路由器
        }
        ...
        peer_as_numbers = set()
        in_peer_table = False  # 标记是否开始解析邻居表
        ...
        for line in lines:
            ...
            for key, value in key_value_matches:
                ...
                if key == 'bgp local router id':
                    ...
                    logging.debug(f"检测到 BGP 本地路由器 ID: {bgp_info['bgp_local_router_id']}")
        ...
        if local_as_number is not None and any(peer_as != local_as_number for peer_as in peer_as_numbers):
            ...
            logging.info("此路由器是边界路由器。")
        else:
            logging.info("此路由器不是边界路由器。")

        # 检查边界路由器的数量是否合理（可以根据实际需求自定义逻辑）
        total_boundary_routers = len([as_num for as_num in peer_as_numbers if as_num != local_as_number])
        if total_boundary_routers > 1:
            logging.warning(f"边界路由器数量: {total_boundary_routers}，可能不太合理。")
        else:
            logging.info(f"边界路由器数量: {total_boundary_routers}，看起来合理。")
        ...
        if bgp_info["bgp_local_router_id"] and bgp_info["bgp_local_as_number"]:
            ...
            logging.warning("未能从 'display bgp all summary' 输出中提取完整的 BGP 信息。")
            return None
