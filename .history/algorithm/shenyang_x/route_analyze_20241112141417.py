...
class LoopDetector:
    def __init__(self, routing_data: Dict[str, Any], ip_to_interface: Dict[str, str]):
        self.routing_data = routing_data
        self.ip_to_interface = ip_to_interface  # IP 到接口的映射
        self.graph: Dict[str, Set[str]] = {}  # 使用字典存储邻接表

    def build_graph(self):
        for sysname, data in self.routing_data.get("telnet_devices", {}).items():
            for route in data.get("routing_table", []):
                next_hop_ip = route.get("NextHop")
                if next_hop_ip and next_hop_ip != "unassigned":
                    # 使用 IP 到接口的映射
                    next_interface = self.ip_to_interface.get(next_hop_ip)
                    if next_interface:
                        # 当前路由器的接口
                        current_interface = f"{sysname}-{route.get('Interface')}"
                        if current_interface not in self.graph:
                            self.graph[current_interface] = set()
                        self.graph[current_interface].add(next_interface)
                        logging.debug(f"Adding edge from {current_interface} to {next_interface}")
                    else:
                        logging.warning(f"NextHop IP {next_hop_ip} for {sysname} does not map to any interface.")
        logging.info("Routing graph constructed.")

    def detect_cycles_util(self, node: str, visited: Set[str], rec_stack: Set[str]) -> bool:
        visited.add(node)
        rec_stack.add(node)
        for neighbour in self.graph.get(node, []):
            if neighbour not in visited:
                if self.detect_cycles_util(neighbour, visited, rec_stack):
                    return True
    ...
    def detect_cycles(self) -> bool:
        ...
        for node in self.graph:
            if node not in visited:
                if self.detect_cycles_util(node, visited, rec_stack):
                    logging.warning("Cycle detected in the routing graph.")
                    return True
        logging.info("No cycles detected in the routing graph.")
        return False
    ...
    def get_cycles(self) -> List[List[str]]:
        ...
        def dfs(current, start, path, visited_local):
            path.append(current)
            visited_local.add(current)
            for neighbour in self.graph.get(current, []):
                if neighbour == start:
                    all_cycles.append(path.copy())
                elif neighbour not in visited_local:
                    dfs(neighbour, start, path, visited_local)
            path.pop()
            visited_local.remove(current)

        for node in self.graph:
            if node not in visited_global:
                dfs(node, node, path, set())
...
def write_report(report_path: str, graph: Dict[str, Set[str]], has_cycles: bool, cycles: List[List[str]],
                 execution_time: str):
    try:
        with open(report_path, 'w') as f:
            ...
            f.write("            路由环路故障检测报告            \n")
            f.write("========================================\n\n")
            f.write(f"执行时间：{execution_time}\n")
            f.write(f"执行结果：{'存在环路' if has_cycles else '成功'}\n\n")
            f.write("执行内容：\n\n")
            f.write("构建的路由图（邻接表表示）：\n")
            for sysname_interface, neighbors in graph.items():
                f.write(f"{sysname_interface} -> {', '.join(neighbors)}\n")
            f.write("\n")
            f.write("环路故障检测结果：\n")
            if has_cycles:
                f.write("检测到以下路由环路：\n")
...
def build_ip_to_interface_mapping(routing_data: Dict[str, Any]) -> Dict[str, str]:
    """
    从路由数据中构建 IP 到接口的映射
    """
    ip_to_interface = {}
    for sysname, data in routing_data.get("telnet_devices", {}).items():
        interfaces = data.get("interfaces", {})
        for interface, ip in interfaces.items():
            if ip and ip != "unassigned":
                ip_addr = ip.split('/')[0]  # 仅取 IP 地址，不包括掩码
                interface_id = f"{sysname}-{interface}"
                ip_to_interface[ip_addr] = interface_id
    logging.info(f"Built IP to Interface mapping: {ip_to_interface}")
...
def main(input_path: str, report_path: str):
    ...
    # 构建 IP 到接口的映射
    ip_to_interface = build_ip_to_interface_mapping(mapping)

    # 进行环路检测
    loop_detector = LoopDetector(mapping, ip_to_interface)
    loop_detector.build_graph()
    has_cycles = loop_detector.detect_cycles()
    cycles = loop_detector.get_cycles() if has_cycles else []

    # 获取构建的图
    graph = loop_detector.graph

    # 生成报告
    write_report(report_path, graph, has_cycles, cycles, execution_time)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process router configurations from param.json and detect routing loops.")
