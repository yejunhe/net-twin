...
class TopologyMapper:
    ...
    def map_topology(self):
        ...
        for node_id, node_info in self.unl_parser.nodes.items():
            ...
            for host_port, sysname in self.router_manager.telnet_sysnames.items():
                if node_name == sysname:
                    mapping[node_id] = {
                        'node_name': node_name,
                        'host_port': host_port,
                        'sysname': sysname,
                        'configuration': self.router_manager.telnet_configurations.get(host_port, 'No config')
                    }

        # Map Docker-based routers
        for docker_id, sysname in self.router_manager.docker_sysnames.items():
            if sysname and sysname != "Unknown":
                mapping[docker_id] = {
                    'docker_id': docker_id,
                    'sysname': sysname,
                    'configuration': self.router_manager.docker_configurations.get(docker_id, 'No config')
                }
