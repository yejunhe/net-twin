...
class RouterManager:
    ...
    def execute_telnet_commands(self, tn, image_type):
        ...
        try:
            ...
            while True:
                ...
                # Determine action based on current prompt
                if image_type == "h3c":
                    if data.endswith("]"):
                        # Send 'quit' command
                        tn.write(b'quit\n')
                        ...
                        # Send 'screen-length disable' command
                    ...
                    break
            # Allow some time for the last command to execute
            time.sleep(1)
            # Read the final output after command execution
    ...
    def connect_and_get_sysnames_and_configs(self):
        ...
        for node in self.telnet_info.get("node", []):
            ...
            # Partial matching for FRR routing, H3C, and Huawei NE40 image types
            if "frrouting" in image_type:
                # Connect via Docker for FRR routing
                docker_id = node.get("dockerid")
                if docker_id:
                    success = self.connect_via_docker(docker_id)
                    if success:
                        # Configuration already retrieved through Docker commands
                        pass
                else:
                    print(f"No Docker ID provided for node with image_type 'frrouting'.")
        
            elif "h3c" in image_type or "h3cvsr" in image_type:
                # Handle any H3C or H3C variant image types
            ...
            elif "huaweine40" in image_type or "ne40" in image_type:
                # Handle any Huawei NE40 or variant image types
                host = node.get("hostip")
                port = node.get("port")
                ...
                print(f"Unsupported or unknown image_type '{image_type}' for node. Skipping.")
