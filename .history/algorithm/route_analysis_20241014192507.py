...
class RouterManager:
    ...
    def execute_telnet_commands(self, tn):
        """
        Execute a predefined list of commands on a Telnet connection.
        This function no longer depends on image_type and sends commands directly.
    ...
    def get_configuration_via_telnet(self, tn):
        """通过 Telnet 获取路由器的配置信息"""
        try:
            # 直接执行命令序列，不再根据 image_type 判断
            config_output = self.execute_telnet_commands(tn)
            ...
            if "frrouting" in image_type:
             # 通过 Docker 执行命令
                ...
                if sysname:
                    ...
                    # 执行后续命令，获取配置信息
                    config = self.get_configuration_via_telnet(tn)
                    if config:
